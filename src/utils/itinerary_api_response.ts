...
function getPostgresError(error: unknown): PostgresErrorLike | null {
  ...
  const causeCandidate = cause as Record<string, unknown>;

  const causeCode = typeof causeCandidate.code === "string" ? causeCandidate.code : undefined;
  ...
  return {
    ...
    constraint: typeof causeCandidate.constraint === "string" ? causeCandidate.constraint : undefined,

    constraint_name: typeof causeCandidate.constraint_name === "string" ? causeCandidate.constraint_name : undefined,
  };
}

function getUniqueConstraintErrors(constraintName: string | undefined): FieldErrors | undefined {
  if (!constraintName) {
    return undefined;
  }
  ...
  if (constraintName.includes("day_number")) {
    errors.dayNumber = ["Số thứ tự ngày đã tồn tại trong hành trình"];

    return errors;
  }

  if (constraintName.includes("sort_order")) {
    errors.sortOrder = ["Thứ tự đã tồn tại"];
...
export function handleItineraryServiceError(error: unknown) {
  ...
  if (postgresError?.code === "23505") {
    ...
    return errorResponse("Dữ liệu bị trùng với bản ghi hiện có", 409, getUniqueConstraintErrors(constraintName));
  }

  if (postgresError?.code === "23503") {
    return errorResponse("Dữ liệu liên quan không tồn tại hoặc đang được sử dụng", 409);
  }
  ...
  console.error("Unhandled itinerary API error", error);

  return errorResponse("Đã xảy ra lỗi khi xử lý hành trình", 500);
}
