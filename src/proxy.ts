...
function getAuthReturnPath(request: NextRequest) {
  const returnPath = normalizeReturnPath(request.nextUrl.searchParams.get("next"), "/");
...
export async function proxy(request: NextRequest) {
  ...
  if (!process.env.NEXT_PUBLIC_SUPABASE_URL || !process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY) {
    console.error("[proxy] Thiếu NEXT_PUBLIC_SUPABASE_URL hoặc NEXT_PUBLIC_SUPABASE_ANON_KEY, bỏ qua kiểm tra auth.");
  ...
  try {
    ...
    if (pathname.startsWith("/admin")) {
      if (!isAuthenticated) {
        const returnPath = getCurrentRequestPath(request);

        return redirectWithCookies(`/auth/login?next=${encodeURIComponent(returnPath)}`);
      }
    ...
    if (isAuthenticated && (pathname === "/auth/login" || pathname === "/auth/register")) {
      return redirectWithCookies(getAuthReturnPath(request));
    }

    return supabaseResponse;
  } catch (error) {
    console.error("[proxy] Lỗi khi kiểm tra auth:", error);
