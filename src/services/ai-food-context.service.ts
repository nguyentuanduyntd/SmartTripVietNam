...
async function loadRestaurantContext(restaurantId: string): Promise<RestaurantContext | null> {
  const [row] = await db
    .select({
      id: restaurants.id,
    ...
    .from(restaurants)
    .innerJoin(locations, eq(restaurants.locationId, locations.id))
  ...
  const cuisineMap = await findCuisinesByRestaurantIds([restaurantId]);
...
function buildPlannerPrompt(context: RestaurantContext) {
  ...
  return [
    ...
    cuisines ? `Món/ẩm thực liên quan: ${cuisines}.` : null,
...
function fallbackAnswer(context: RestaurantContext, question: string) {
  ...
  if (/tre|be|con|gia dinh/.test(normalized)) {
  ...
  if (/gia|ngan sach|bao nhieu|chi phi/.test(normalized)) {
    return `Khoảng chi tiêu tham khảo trong dữ liệu SmartTrip là ${formatMoney(context.priceMin)} - ${formatMoney(context.priceMax)} mỗi người. Đây là dữ liệu ${context.source === "demo" ? "demo" : "tham khảo"}, không phải giá realtime.`;
  }
  ...
  const cuisines = context.cuisines.map((item) => item.name).join(", ");

  return `${context.name} là một lựa chọn trong dữ liệu SmartTrip tại ${context.locationName}. ${
    cuisines ? `Các món/ẩm thực liên quan gồm ${cuisines}. ` : ""
  }Khoảng giá tham khảo ${formatMoney(context.priceMin)} - ${formatMoney(context.priceMax)} mỗi người. Bạn có thể hỏi mình cụ thể về trẻ nhỏ, độ cay, ngân sách hoặc thời điểm nên ghé.`;
}
  ...
  return [
    {
      ...
      question: "Quán này có phù hợp nếu tôi đi cùng trẻ 5 tuổi không?",
    },
    {
      label: "Món nào nên thử?",
      question: "Ở quán này nên ưu tiên thử món nào và vì sao?",
    },
    {
      label: "Ăn tối muộn được không?",
      question: "Quán này có phù hợp để ăn tối muộn không?",
    },
    ...(context.priceMin !== null || context.priceMax !== null
      ? [
          {
            label: "Ngân sách bao nhiêu?",
            question: "Khoảng ngân sách hợp lý cho một người ở quán này là bao nhiêu?",
          },
        ]
      : []),
  ].slice(0, 4);
...
export async function askFoodContextAiService(input: AiFoodContextRequest) {
  const context = await loadRestaurantContext(input.entityId);

  if (!context) {
    throw new Error("Không tìm thấy quán ăn trong dữ liệu SmartTrip.");
  }
...
CUISINES:
${context.cuisines.map((item) => `- ${item.name} | avgPrice=${item.avgPrice ?? "unknown"} | signature=${item.isSignature}`).join("\n") || "none"}
...
`.trim();
  ...
  let generatedBy: "gemini" | "fallback" = "fallback";
  let answer = fallbackAnswer(context, input.question);
  let quickReplies = defaultQuickReplies(context);

  try {
    const raw = await generateGeminiLooseJson({ prompt });
    ...
    if (parsed.success) {
      ...
      quickReplies = parsed.data.quickReplies?.length ? parsed.data.quickReplies : quickReplies;
    }
  } catch (error) {
    console.warn("[AI FOOD CONTEXT] Gemini unavailable, using fallback", error);
  }
  ...
  return {
    entity: {
      ...
      isFamilyFriendly: context.isFamilyFriendly,
      isOpenLate: context.isOpenLate,
      cuisines: context.cuisines.map((item) => ({
        id: item.id,
        name: item.name,
        slug: item.slug,
