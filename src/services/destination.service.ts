...
const CUISINE_CACHE_PREFIX = "smarttrip:v1:cuisines:";

async function invalidateDestinationCache() {
  await Promise.all([deleteCacheByPrefix(DESTINATION_CACHE_PREFIX), deleteCacheByPrefix(CUISINE_CACHE_PREFIX)]);
}
...
async function attachCategories<T extends { id: string }>(rows: T[]) {
  const categoriesMap = await findCategoriesByDestinationIds(rows.map((row) => row.id));
...
export async function listDestinations(filters: DestinationFilters) {
  const key = `${DESTINATION_CACHE_PREFIX}list:` + buildCacheKey(filters);

  return rememberCachedValue(key, CACHE_TTL_SECONDS.short, async () => {
...
export async function getDestinationById(id: string) {
  return rememberCachedValue(`${DESTINATION_CACHE_PREFIX}detail:${id}`, CACHE_TTL_SECONDS.medium, async () => {
...
export async function createDestinationService(input: CreateDestinationRequest) {
...
export async function updateDestinationService(id: string, input: UpdateDestinationRequest) {
  ...
  const coverWasChanged = input.coverImagePublicId !== undefined && input.coverImagePublicId !== oldCoverPublicId;
