...
function makeMissingReply(state: PlannerConversationStateInput): {
  reply: string;
  quickReplies: TravelQuickReply[];
} {
  if (!state.locationId) {
    return {
      reply: "Bạn muốn đi đâu? Cứ nói tên thành phố/khu vực và kiểu trải nghiệm bạn thích nhé.",
  ...
  if (!state.adultCount) {
    return {
      reply: `${state.locationName ?? "Điểm đến này"} ổn đó 👍 Bạn đi bao nhiêu người? Nếu có trẻ em thì nói luôn để mình sắp lịch nhẹ hơn.`,

      quickReplies: [
        {
          label: "1 người",
          value: "Tôi đi 1 người",
          action: "send",
        },
        {
          label: "2 người",
          value: "Tôi đi 2 người",
          action: "send",
        },
        {
          label: "4 người",
          value: "Tôi đi 4 người",
          action: "send",
        },
      ],
    };
  ...
  if (!state.dayCount && !state.startDate) {
    return {
      reply: "Bạn dự định đi mấy ngày và khởi hành khi nào? Ví dụ: “3 ngày, thứ Sáu tuần sau”.",
  ...
  if (!state.dayCount) {
    return {
      reply: "Bạn muốn chuyến đi kéo dài mấy ngày?",
  ...
  return {
    reply: "Bạn dự định khởi hành ngày nào? Bạn có thể nói “20/08/2026”, “ngày mai” hoặc “thứ Sáu tuần sau”.",
...
function buildReadyReply(state: PlannerConversationStateInput) {
  const travelers = (state.adultCount ?? 0) + (state.childCount ?? 0);
...
export async function handleAiTravelChatService(input: AiTravelChatRequest): Promise<TravelChatServerResponse> {
  ...
  if (!readyToGenerate) {
    const missing = makeMissingReply(state);
    ...
    return {
      ...
      quickReplies: missing.quickReplies,
    };
  }
  ...
  const shouldGenerate = (!wasReady && readyToGenerate) || userExplicitlyAskedToGenerate;
  ...
  return {
    ...
    action: shouldGenerate ? "generate" : "none",

    readyToGenerate: true,

    quickReplies: shouldGenerate
      ? []
      : [
          {
