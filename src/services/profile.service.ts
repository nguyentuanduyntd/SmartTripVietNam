...
import { CLOUDINARY_FOLDERS, deleteImage, uploadImage } from "@/src/services/image.service";
...
export async function updateProfileBasics(userId: string, input: UpdateProfileInput) {
  const profile = await saveProfileBasics(userId, input);
...
export async function replaceProfileAvatar(userId: string, file: File) {
