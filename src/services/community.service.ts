...
import { deleteImage, uploadImage, CLOUDINARY_FOLDERS } from "@/src/services/image.service";
...
import type { CommunityItinerarySnapshot } from "@/src/db/schema/community";
...
export class CommunityServiceError extends Error {
  constructor(
    message: string,
    public readonly status: 400 | 401 | 403 | 404 | 422,
  ) {
    super(message);
...
function getDayOffset(baseDate: string | null, targetDate: string) {
  if (!baseDate) {
    return null;
  }

  return Math.round((parseDateToUtc(targetDate) - parseDateToUtc(baseDate)) / (24 * 60 * 60 * 1000));
}

function getInclusiveDayCount(startDate: string, endDate: string) {
  return Math.round((parseDateToUtc(endDate) - parseDateToUtc(startDate)) / (24 * 60 * 60 * 1000)) + 1;
}

function buildCommunitySnapshot(
  planner: NonNullable<Awaited<ReturnType<typeof findUserItineraryPlannerDetailById>>>,
): CommunityItinerarySnapshot {
  const dayNumberById = new Map(planner.days.map((day) => [day.id, day.dayNumber]));

  const itemScopeById = new Map<string, { dayNumber: number; sortOrder: number }>();

  const mealScopeById = new Map<string, { dayNumber: number; sortOrder: number }>();
  ...
  return {
    ...
    stays: planner.stays.map((stay) => ({
      ...
      checkInDayOffset: getDayOffset(planner.startDate, stay.checkInDate),
      checkOutDayOffset: getDayOffset(planner.startDate, stay.checkOutDate),
    ...
    costs: planner.costs.map((cost) => {
      const itemScope = cost.itineraryItemId ? itemScopeById.get(cost.itineraryItemId) : undefined;

      const mealScope = cost.itineraryMealId ? mealScopeById.get(cost.itineraryMealId) : undefined;

      const directDayNumber = cost.itineraryDayId ? (dayNumberById.get(cost.itineraryDayId) ?? null) : null;
      ...
      return {
        ...
        scope: {
          dayNumber: itemScope?.dayNumber ?? mealScope?.dayNumber ?? directDayNumber,
...
export async function createCommunityPostService(input: {
  ...
  if (sourceItineraryId) {
    const planner = await findUserItineraryPlannerDetailById(sourceItineraryId, input.userId);

    if (!planner) {
      notFound("Không tìm thấy lịch trình hoặc lịch trình không thuộc tài khoản của bạn");
    }
    ...
    tripEndDate =
      planner.startDate && planner.days.length > 0 ? addDays(planner.startDate, planner.days.length - 1) : null;
    estimatedCost = planner.costSummary.total;
    destinationIds = getPlannerDestinationIds(planner);
  } else {
    if (!dayCount && tripStartDate && tripEndDate) {
      dayCount = getInclusiveDayCount(tripStartDate, tripEndDate);
    }
  }

  const existingDestinationIds = await findExistingDestinationIds(destinationIds);

  if (new Set(existingDestinationIds).size !== new Set(destinationIds).size) {
...
export async function getCommunityFeedService(query: CommunityFeedQuery, currentUserId: string | null) {
...
export async function getCommunityPostService(postId: string, currentUserId: string | null) {
  const post = await findPublicCommunityPostById(postId, currentUserId);
...
export async function updateCommunityPostService(postId: string, userId: string, data: UpdateCommunityPostInput) {
  ...
  const updated = await updateOwnedCommunityPost(postId, userId, data);
...
export async function deleteCommunityPostService(postId: string, userId: string) {
  const owned = await findOwnedCommunityPost(postId, userId);
  ...
  const deleted = await softDeleteOwnedCommunityPost(postId, userId);

  if (!deleted) {
    notFound("Không tìm thấy bài chia sẻ");
  }

  const cleanupResults = await Promise.allSettled(owned.imagePublicIds.map((publicId) => deleteImage(publicId)));

  for (const result of cleanupResults) {
    if (result.status === "rejected") {
      console.error("Không thể xóa ảnh community trên Cloudinary:", result.reason);
    }
  }
...
export async function setCommunityPostLikeService(postId: string, userId: string, active: boolean) {
  await ensurePublicPost(postId);
  return setCommunityPostLike(postId, userId, active);
}

export async function setCommunityPostSaveService(postId: string, userId: string, active: boolean) {
  await ensurePublicPost(postId);
...
export async function createCommunityCommentService(input: {
  ...
  if (input.parentId) {
    ...
    if (!parent || parent.postId !== input.postId || parent.status !== "approved" || parent.deletedAt) {
    ...
    if (parent.parentId) {
      ...
      if (!root || root.postId !== input.postId || root.status !== "approved" || root.deletedAt) {
...
async function requireOwnedComment(commentId: string, userId: string) {
...
export async function updateCommunityCommentService(commentId: string, userId: string, content: string) {
  const comment = await requireOwnedComment(commentId, userId);
  await ensurePublicPost(comment.postId);

  const updated = await updateOwnedCommunityComment(commentId, userId, content.trim());
...
export async function deleteCommunityCommentService(commentId: string, userId: string) {
  const comment = await requireOwnedComment(commentId, userId);
  await ensurePublicPost(comment.postId);

  const deleted = await softDeleteOwnedCommunityComment(commentId, userId);
