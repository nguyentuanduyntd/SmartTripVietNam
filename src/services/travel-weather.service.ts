...
async function geocodeLocation(locationName: string) {
  ...
  if (!response.ok) {
    throw new Error(`Open-Meteo geocoding lỗi ${response.status}`);
  }
  ...
  const target = normalize(locationName).trim();

  return (
    rows.find((row) => row.country_code === "VN" && normalize(row.name).trim() === target) ??
...
export async function checkTravelWeatherService(input: WeatherInput): Promise<TravelWeatherResult> {
  ...
  if (offset < 0) {
    ...
    return {
      ...
      message: "Ngày khởi hành đã ở trong quá khứ nên Weather Forecast không được dùng cho lịch này.",
    };
  }
  ...
  for (const activity of input.activities) {
    ...
    const time = `${date}T${activity.startTime.padStart(5, "0")}`;
