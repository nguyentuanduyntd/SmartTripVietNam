...
async function invalidateCuisineCache() {
  await deleteCacheByPrefix(CUISINE_CACHE_PREFIX);
}
...
async function attachDestinations<T extends { id: string }>(rows: T[]) {
  const destinationsMap = await findDestinationsByCuisineIds(rows.map((row) => row.id));
...
export async function listCuisines(filters: CuisineFilters) {
  const key = `${CUISINE_CACHE_PREFIX}list:` + buildCacheKey(filters);
...
export async function getCuisineById(id: string) {
  return rememberCachedValue(`${CUISINE_CACHE_PREFIX}detail:${id}`, CACHE_TTL_SECONDS.medium, async () => {
...
export async function updateCuisineService(id: string, input: UpdateCuisineRequest) {
