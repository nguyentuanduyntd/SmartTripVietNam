...
export const ADMIN_COMMUNITY_MODERATION_ERROR_CODES = {
  REPORT_NOT_FOUND: "ADMIN_COMMUNITY_REPORT_NOT_FOUND",
  REPORT_ALREADY_REVIEWED: "ADMIN_COMMUNITY_REPORT_ALREADY_REVIEWED",
  TARGET_NOT_FOUND: "ADMIN_COMMUNITY_TARGET_NOT_FOUND",
} as const;
...
export class AdminCommunityModerationError extends Error {
  readonly code: AdminCommunityModerationErrorCode;

  readonly status: number;

  constructor(code: AdminCommunityModerationErrorCode, message: string, status: number) {
    super(message);

    this.name = "AdminCommunityModerationError";
    this.code = code;
    this.status = status;
  }
}

function normalizeReviewNote(value: string | undefined) {
...
export async function getCommunityReportsForAdmin(input: AdminCommunityReportListQuery) {
  const result = await listCommunityReportsForAdmin(input);

  return {
    ...result,

    rows: result.rows.map((row) => {
      const target = row.post?.id
        ? {
            type: "post" as const,
            id: row.post.id,
            postId: row.post.id,
            ...
            author: row.postAuthor?.id ? row.postAuthor : null,
          }
        : row.comment?.id
          ? {
              ...
              author: row.commentAuthor?.id ? row.commentAuthor : null,
            }
          : null;
      ...
      return {
        ...
        reviewer: row.reviewer?.id ? row.reviewer : null,
...
export async function moderateCommunityReport(adminId: string, reportId: string, input: ModerateCommunityReportInput) {
  const reviewNote = normalizeReviewNote(input.reviewNote);

  if (input.action === "dismiss") {
    const result = await dismissCommunityReport(reportId, adminId, reviewNote);
  ...
  const result = await resolveCommunityReport(reportId, adminId, reviewNote);
