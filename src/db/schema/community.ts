...
import { communityReportReasonEnum, communityReportStatusEnum, socialContentStatusEnum } from "./tour_community_enums";
...
export const communityPosts = pgTable(
  "community_posts",
  {
    ...
    sourceItineraryId: uuid("source_itinerary_id").references(() => userItineraries.id, {
      onDelete: "set null",
    }),
    ...
    itinerarySnapshot: jsonb("itinerary_snapshot").$type<CommunityItinerarySnapshot | null>(),
  ...
  (table) => [
    index("community_posts_status_created_at_idx").on(table.status, table.createdAt),

    index("community_posts_user_id_idx").on(table.userId),
    index("community_posts_tour_id_idx").on(table.tourId),
    index("community_posts_source_itinerary_id_idx").on(table.sourceItineraryId),
    index("community_posts_location_id_idx").on(table.locationId),

    check("community_posts_content_check", sql`length(btrim(${table.content})) > 0`),
    ...
    check("community_posts_day_count_check", sql`${table.dayCount} is null or ${table.dayCount} > 0`),

    check("community_posts_estimated_cost_check", sql`${table.estimatedCost} is null or ${table.estimatedCost} >= 0`),
...
export const postImages = pgTable(
  ...
  (table) => [
    uniqueIndex("post_images_post_sort_order_uidx").on(table.postId, table.sortOrder),

    index("post_images_post_id_idx").on(table.postId),

    check("post_images_sort_order_check", sql`${table.sortOrder} >= 0`),

    check("post_images_width_check", sql`${table.width} is null or ${table.width} > 0`),

    check("post_images_height_check", sql`${table.height} is null or ${table.height} > 0`),
  ],
);
...
export const postDestinations = pgTable(
  ...
  (table) => [
    primaryKey({
      columns: [table.postId, table.destinationId],
    }),
...
export const postComments = pgTable(
  ...
  (table) => [
    ...
    check("post_comments_content_check", sql`length(btrim(${table.content})) > 0`),
  ],
);
...
export const communityReports = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex("community_reports_reporter_post_uidx").on(table.reporterId, table.postId),

    uniqueIndex("community_reports_reporter_comment_uidx").on(table.reporterId, table.commentId),

    index("community_reports_status_created_at_idx").on(table.status, table.createdAt),

    index("community_reports_reporter_id_idx").on(table.reporterId),

    index("community_reports_post_id_idx").on(table.postId),

    index("community_reports_comment_id_idx").on(table.commentId),

    index("community_reports_reviewed_by_idx").on(table.reviewedBy),
  ],
);
