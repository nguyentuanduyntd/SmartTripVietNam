...
export const destinationsToCategoies = pgTable(
  "destinations_to_categories",
  {
    ...
    categoryId: uuid("category_id")
      .notNull()
      .references(() => destinationCategories.id, { onDelete: "cascade" }),
  },
  (table) => [
    primaryKey({ columns: [table.destinationId, table.categoryId] }),

    index("destinations_to_categories_destination_id_idx").on(table.destinationId),

    index("destinations_to_categories_category_id_idx").on(table.categoryId),
  ],
);
...
export const destinationImages = pgTable(
  ...
  (table) => [
    uniqueIndex("destination_images_public_id_unique").on(table.publicId),
