...
export const cuisinesToDestinations = pgTable(
  "cuisines_to_destinations",
  {
  ...
  (table) => [
    primaryKey({ columns: [table.cuisineId, table.destinationId] }),
...
export const cuisineEmbeddings = pgTable(
  "cuisine_embeddings",
  {
    id: uuid("id").primaryKey().defaultRandom(),
  ...
  (table) => [index("cuisine_embedding_vector_idx").using("hnsw", table.embedding.op("vector_cosine_ops"))],
);
