...
type GetAdminStats = (typeof import("@/src/services/admin-stats.service"))["getAdminStats"];
...
describe("PostgreSQL integration", () => {
  ...
  beforeAll(async () => {
    ...
    const adminStatsModule = await import("@/src/services/admin-stats.service");
  ...
  it("kết nối PostgreSQL và thực hiện select 1", async () => {
    const rows = await db.execute<{ ok: number }>(sql`select 1::int as ok`);
  ...
  it("có đầy đủ các bảng cần cho admin stats", async () => {
    const rows = await db.execute<{ tableName: string }>(sql`
        ...
        `);
    const actualTableNames = new Set(rows.map((row) => row.tableName));

    expect(actualTableNames).toEqual(new Set(REQUIRED_ADMIN_STATS_TABLES));
  });
