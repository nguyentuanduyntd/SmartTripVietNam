...
export async function findDestinationCategoryLinks() {
  return db
    ...
    .innerJoin(destinationCategories, eq(destinationsToCategoies.categoryId, destinationCategories.id));
}
...
export async function replaceDestinationEmbeddings(
  ...
  await db.transaction(async (tx) => {
    await tx.delete(destinationEmbeddings).where(eq(destinationEmbeddings.destinationId, destinationId));

    if (chunks.length === 0) {
      return;
    }

    await tx.insert(destinationEmbeddings).values(
...
export async function replaceCuisineEmbeddings(
  ...
  await db.transaction(async (tx) => {
    await tx.delete(cuisineEmbeddings).where(eq(cuisineEmbeddings.cuisineId, cuisineId));

    if (chunks.length === 0) {
      return;
    }

    await tx.insert(cuisineEmbeddings).values(
      chunks.map((chunk, chunkIndex) => ({
        cuisineId,
        chunkIndex,
        content: chunk.content,
...
export async function searchDestinationEmbeddings(
  ...
  const similarity = sql<number>`
            1 - (
                ${cosineDistance(destinationEmbeddings.embedding, queryEmbedding)}
            )
        `;

  const conditions = [gt(similarity, options.minSimilarity)];

  if (options.locationId) {
    conditions.push(eq(destinations.locationId, options.locationId));
  }

  return db
    .select({
      kind: sql<"destination">`
                    'destination'
                `,
      ...
      chunkIndex: destinationEmbeddings.chunkIndex,

      content: destinationEmbeddings.content,

      similarity,
    })
    .from(destinationEmbeddings)
    .innerJoin(destinations, eq(destinationEmbeddings.destinationId, destinations.id))
    .innerJoin(locations, eq(destinations.locationId, locations.id))
...
export async function findCuisineIdsByLocationId(locationId: string) {
  const rows = await db
    .select({
      cuisineId: cuisinesToDestinations.cuisineId,
    })
  ...
  return [...new Set(rows.map((row) => row.cuisineId))];
}
...
export async function searchCuisineEmbeddings(
  ...
  const similarity = sql<number>`
            1 - (
                ${cosineDistance(cuisineEmbeddings.embedding, queryEmbedding)}
            )
        `;
  ...
  if (options.locationId) {
    const cuisineIds = await findCuisineIdsByLocationId(options.locationId);

    if (cuisineIds.length === 0) {
      return [];
    }

    conditions.push(inArray(cuisineEmbeddings.cuisineId, cuisineIds));
  }
  ...
  return db
    .select({
      ...
      chunkIndex: cuisineEmbeddings.chunkIndex,

      content: cuisineEmbeddings.content,

      similarity,
    })
    .from(cuisineEmbeddings)
    .innerJoin(cuisines, eq(cuisineEmbeddings.cuisineId, cuisines.id))
    .where(and(...conditions))
    .orderBy(desc(similarity))
    .limit(options.limit);
}
