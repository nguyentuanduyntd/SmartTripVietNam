...
import { communityPostDeletionLogs, notifications } from "@/src/db/schema/notifications";
...
function buildStoryConditions(input: AdminStoryListQuery) {
  const conditions: SQL[] = [isNull(communityPosts.deletedAt)];
...
export async function deleteStoryPermanently(storyId: string, adminId: string, reason: string) {
  return db.transaction(async (tx) => {
    ...
    await tx.delete(communityPosts).where(eq(communityPosts.id, storyId));
