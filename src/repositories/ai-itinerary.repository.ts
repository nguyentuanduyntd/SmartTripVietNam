...
export async function findAiPlannerLocationById(locationId: string) {
...
export async function findAiDestinationsByIds(ids: string[]) {
  if (ids.length === 0) {
    return [];
  }
...
export async function findAiCuisinesByIds(ids: string[]) {
  if (ids.length === 0) {
    return [];
  }
...
export async function createAiItinerary(input: {
  userId: string;
  ...
  return db.transaction(async (tx) => {
    const [itinerary] = await tx
      ...
      .values({
        ...
        startLocationId: input.location.id,

        startLocationName: input.location.name,
    ...
    if (!itinerary) {
      throw new Error("Không thể tạo hành trình AI.");
    }
    ...
    for (const day of input.plan.days) {
      ...
      if (!createdDay) {
        throw new Error(`Không thể tạo ngày ${day.dayNumber}.`);
      }
      ...
      for (let mealIndex = 0; mealIndex < day.meals.length; mealIndex++) {
        ...
        if (meal.cuisines.length > 0) {
          await tx.insert(itineraryMealCuisines).values(
            meal.cuisines.map((cuisine, cuisineIndex) => ({
              itineraryMealId: createdMeal.id,

              cuisineId: cuisine.cuisineId,

              cuisineName: cuisine.cuisineName,

              sortOrder: cuisineIndex,
    ...
    if (nonAccommodationCosts.length > 0) {
      await tx.insert(itineraryCosts).values(
        nonAccommodationCosts.map((cost, index) => ({
          ...
          unitPrice: String(Math.round(cost.unitPrice)),

          quantity: String(cost.quantity),

          nightCount:
            cost.calculationUnit === "per_room" ? (cost.nightCount ?? Math.max(input.request.dayCount - 1, 1)) : null,

          note: cost.note || "Chi phí dự kiến do AI đề xuất.",
