...
async function hydrateCommunityPostCards(
  ...
  const [images, mentionedDestinations, likedRows, savedRows] = await Promise.all([
    db
      .select({
        id: postImages.id,
    ...
    db
      ...
      .innerJoin(destinations, eq(postDestinations.destinationId, destinations.id))
    ...
    !currentUserId
      ...
      : db
          ...
          .from(postSaves)
          .where(and(inArray(postSaves.postId, postIds), eq(postSaves.userId, currentUserId))),
  ]);
  ...
  const destinationsByPostId = new Map<string, typeof mentionedDestinations>();
...
export async function findExistingDestinationIds(ids: string[]) {
  ...
  const rows = await db.select({ id: destinations.id }).from(destinations).where(inArray(destinations.id, uniqueIds));

  return rows.map((row) => row.id);
}

export async function createCommunityPostRecord(input: CreateCommunityPostRecordInput) {
...
export async function findCommunityFeed(options: CommunityFeedRepositoryOptions) {
  ...
  const conditions = [eq(communityPosts.status, "approved"), isNull(communityPosts.deletedAt)];
  ...
  const rows =
    options.sort === "popular"
      ? await baseQuery.orderBy(desc(likeCount), desc(communityPosts.createdAt)).limit(options.limit).offset(offset)
      : await baseQuery.orderBy(desc(communityPosts.createdAt)).limit(options.limit).offset(offset);

  const hydrated = await hydrateCommunityPostCards(rows, options.currentUserId);
...
export async function findPublicCommunityPostById(postId: string, currentUserId: string | null) {
  const likeCount = sql<number>`(
        select count(*)::int
        from ${postLikes}
        where ${postLikes.postId} = ${communityPosts.id}
  ...
  const [hydrated] = await hydrateCommunityPostCards([row], currentUserId);
...
export async function findOwnedCommunityPost(postId: string, userId: string) {
  const [post] = await db
    .select({
      id: communityPosts.id,
      userId: communityPosts.userId,
    ...
    .from(communityPosts)
    .where(and(eq(communityPosts.id, postId), eq(communityPosts.userId, userId), isNull(communityPosts.deletedAt)))
    .limit(1);
...
export async function softDeleteOwnedCommunityPost(postId: string, userId: string) {
  const [post] = await db
    .update(communityPosts)
    .set({
      deletedAt: new Date(),
...
export async function setCommunityPostLike(postId: string, userId: string, active: boolean) {
  if (active) {
  ...
  const [row] = await db.select({ value: count() }).from(postLikes).where(eq(postLikes.postId, postId));
...
export async function setCommunityPostSave(postId: string, userId: string, active: boolean) {
  if (active) {
  ...
  const [row] = await db.select({ value: count() }).from(postSaves).where(eq(postSaves.postId, postId));
...
export async function findCommunityPostComments(postId: string) {
  return db
    ...
    .where(and(eq(postComments.postId, postId), eq(postComments.status, "approved"), isNull(postComments.deletedAt)))
...
export async function updateOwnedCommunityComment(commentId: string, userId: string, content: string) {
  const [comment] = await db
    .update(postComments)
    .set({
      content,
    ...
    .where(and(eq(postComments.id, commentId), eq(postComments.userId, userId), isNull(postComments.deletedAt)))
    .returning({
      id: postComments.id,
      content: postComments.content,
      updatedAt: postComments.updatedAt,
...
export async function softDeleteOwnedCommunityComment(commentId: string, userId: string) {
  const [comment] = await db
    .update(postComments)
    .set({
      deletedAt: new Date(),
