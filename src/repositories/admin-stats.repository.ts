...
import { communityPosts, communityReports, postDestinations } from "@/src/db/schema/community";
...
async function countCreatedInRange(table: typeof destinations | typeof cuisines | typeof profiles, range: DateRange) {
  const [row] = await db
    .select({ value: count() })
    .from(table)
    .where(and(gte(table.createdAt, range.start), lt(table.createdAt, range.end)));
...
export async function getAdminStatsData(currentRange: DateRange, previousRange: DateRange) {
  ...
  const [
    ...
    db.select({ value: count() }).from(communityPosts).where(isNull(communityPosts.deletedAt)),
    ...
    db
      ...
      .from(profiles)
      .where(and(gte(profiles.createdAt, currentRange.start), lt(profiles.createdAt, currentRange.end)))
      .groupBy(userDay)
      .orderBy(userDay),
    db
      .select({
      ...
      .innerJoin(communityPosts, eq(postDestinations.postId, communityPosts.id))
      .where(
        and(
          isNull(communityPosts.deletedAt),
          gte(communityPosts.createdAt, currentRange.start),
    ...
    db
      ...
      .innerJoin(communityPosts, eq(communityReports.postId, communityPosts.id))
      ...
      .groupBy(communityPosts.id, communityPosts.title, profiles.fullName)
      .orderBy(desc(count()))
      .limit(5),
    db
      .select({
