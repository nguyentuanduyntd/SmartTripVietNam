...
export async function clonePublishedTourToItinerary(
  ...
  return db.transaction(async (transaction) => {
    const [sourceTour] = await transaction
      ...
      .where(and(eq(tours.id, input.sourceTourId), eq(tours.status, "published")))
      .limit(1);
      ...
      sourceDayIds.length === 0
        ...
        : await Promise.all([
            transaction
              ...
              .where(inArray(tourItems.tourDayId, sourceDayIds))
              .orderBy(asc(tourItems.tourDayId), asc(tourItems.sortOrder), asc(tourItems.id)),

            transaction
              .select()
              .from(tourMeals)
              .where(inArray(tourMeals.tourDayId, sourceDayIds))
    ...
    const sourceMealCuisines =
      sourceMealIds.length === 0
        ...
        : await transaction
            ...
            .where(inArray(tourMealCuisines.tourMealId, sourceMealIds))
            .orderBy(asc(tourMealCuisines.tourMealId), asc(tourMealCuisines.sortOrder));

    const sourceCosts = await transaction
      .select()
      .from(tourCosts)
      .where(eq(tourCosts.tourId, sourceTour.id))
    ...
    const [createdItinerary] = await transaction
      ...
      .values({
        ...
        coverImageUrl: sourceTour.coverImageUrl,
        coverImagePublicId: sourceTour.coverImagePublicId,
        startDate: input.startDate,
        adultCount: input.adultCount,
        childCount: input.childCount,
        roomCount: input.roomCount,
        startLocationId: sourceTour.startLocationId,
        startLocationName: sourceTour.startLocationName,
    ...
    if (!createdItinerary) {
      throw new Error("Không thể tạo hành trình cá nhân");
    }
    ...
    const createdDayIdByNumber = new Map(createdDays.map((day) => [createDayKey(day.dayNumber), day.id]));

    const sourceDayById = new Map<string, SourceTourDay>(sourceDays.map((day) => [day.id, day]));

    function getCreatedDayId(sourceTourDayId: string) {
      const sourceDay = sourceDayById.get(sourceTourDayId);

      if (!sourceDay) {
        throw new Error(`Không tìm thấy ngày nguồn ${sourceTourDayId}`);
      }

      const createdDayId = createdDayIdByNumber.get(createDayKey(sourceDay.dayNumber));

      if (!createdDayId) {
        throw new Error(`Không thể ánh xạ ngày ${sourceDay.dayNumber}`);
      }
        ...
        : await transaction
            ...
            .values(
              sourceItems.map((item) => ({
                itineraryDayId: getCreatedDayId(item.tourDayId),
    ...
    const createdItemIdByKey = new Map(
      createdItems.map((item) => [createItemKey(item.itineraryDayId, item.sortOrder), item.id]),
    );

    const sourceItemById = new Map(sourceItems.map((item) => [item.id, item]));

    function getCreatedItemId(sourceTourItemId: string) {
      const sourceItem = sourceItemById.get(sourceTourItemId);

      if (!sourceItem) {
        throw new Error(`Không tìm thấy hoạt động nguồn ${sourceTourItemId}`);
      }

      const itineraryDayId = getCreatedDayId(sourceItem.tourDayId);

      const createdItemId = createdItemIdByKey.get(createItemKey(itineraryDayId, sourceItem.sortOrder));

      if (!createdItemId) {
        throw new Error(`Không thể ánh xạ hoạt động ${sourceTourItemId}`);
      }
        ...
        : await transaction
            ...
            .values(
              sourceMeals.map((meal) => ({
                itineraryDayId: getCreatedDayId(meal.tourDayId),
    ...
    const createdMealIdByKey = new Map(
      createdMeals.map((meal) => [createMealKey(meal.itineraryDayId, meal.sortOrder), meal.id]),
    );

    const sourceMealById = new Map<string, SourceTourMeal>(sourceMeals.map((meal) => [meal.id, meal]));

    function getCreatedMealId(sourceTourMealId: string) {
      const sourceMeal = sourceMealById.get(sourceTourMealId);

      if (!sourceMeal) {
        throw new Error(`Không tìm thấy bữa ăn nguồn ${sourceTourMealId}`);
      }

      const itineraryDayId = getCreatedDayId(sourceMeal.tourDayId);

      const createdMealId = createdMealIdByKey.get(createMealKey(itineraryDayId, sourceMeal.sortOrder));

      if (!createdMealId) {
        throw new Error(`Không thể ánh xạ bữa ăn ${sourceTourMealId}`);
      }
    ...
    if (sourceMealCuisines.length > 0) {
      await transaction.insert(itineraryMealCuisines).values(
        sourceMealCuisines.map((mealCuisine) => ({
          itineraryMealId: getCreatedMealId(mealCuisine.tourMealId),
          cuisineId: mealCuisine.cuisineId,
          cuisineName: mealCuisine.cuisineName,
          sortOrder: mealCuisine.sortOrder,
    ...
    if (sourceCosts.length > 0) {
      await transaction.insert(itineraryCosts).values(
        sourceCosts.map((cost) => ({
          itineraryId: createdItinerary.id,
          itineraryDayId: cost.tourDayId ? getCreatedDayId(cost.tourDayId) : null,
          itineraryItemId: cost.tourItemId ? getCreatedItemId(cost.tourItemId) : null,
          itineraryMealId: cost.tourMealId ? getCreatedMealId(cost.tourMealId) : null,
          title: cost.title,
      ...
      const legacyCost: typeof itineraryCosts.$inferInsert = {
        itineraryId: createdItinerary.id,
      ...
      await transaction.insert(itineraryCosts).values(legacyCost);
    ...
    return {
      ...
      copied: {
        ...
        cuisines: sourceMealCuisines.length,
...
export async function findUserItineraryById(itineraryId: string, userId: string) {
  const [itinerary] = await db
    .select()
    .from(userItineraries)
    .where(and(eq(userItineraries.id, itineraryId), eq(userItineraries.userId, userId)))
    .limit(1);

  return itinerary ?? null;
}

export async function deleteUserItineraryById(itineraryId: string, userId: string) {
...
export async function userOwnsItinerary(itineraryId: string, userId: string) {
  const [itinerary] = await db
    .select({
      id: userItineraries.id,
    })
