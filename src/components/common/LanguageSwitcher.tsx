...
export function LanguageSwitcher({ className = "", compact = false }: LanguageSwitcherProps) {
  ...
  const currentLocale: AppLocale = isAppLocale(locale) ? locale : "vi";

  async function changeLocale(nextLocale: AppLocale) {
    if (nextLocale === currentLocale || switchingTo) {
      return;
    }
  ...
  return (
    ...
    >
      ...
      <LanguageButton
        locale="vi"
        label="VI"
        title={t("switchToVietnamese")}
        active={currentLocale === "vi"}
        loading={switchingTo === "vi"}
  ...
  return (
    <button
      ...
      className={`inline-flex h-8 min-w-10 items-center justify-center rounded-full px-2.5 text-xs font-extrabold transition ${
        active ? "bg-[#173a3b] text-white" : "text-[#667570] hover:bg-[#f5eee5]"
