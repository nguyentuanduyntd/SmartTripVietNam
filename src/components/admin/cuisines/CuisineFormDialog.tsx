...
import { FormField, TextArea, TextInput } from "@/src/components/ui/FormField";
...
function createInitialForm(initialValue: Cuisine | null): CuisineFormState {
  if (!initialValue) {
    return {
      ...EMPTY_FORM,
      destinationIds: [],
  ...
  return {
    ...
    avgPrice: initialValue.avgPrice !== null ? String(initialValue.avgPrice) : "",
    destinationIds: initialValue.destinations.map((destination) => destination.id),
  };
}
...
export function CuisineFormDialog({
  ...
  function updateField<Key extends keyof CuisineFormState>(key: Key, value: CuisineFormState[Key]) {
    setForm((current) => ({
      ...current,
      [key]: value,
    }));
  ...
  return (
    <Dialog
      open={open}
      title={initialValue ? "Sửa món ăn" : "Thêm món ăn mới"}
      size="lg"
      onClose={onClose}
    ...
    >
      <form id="cuisine-form" className="space-y-4" onSubmit={handleSubmit}>
        <FormField htmlFor="cuisine-name" label="Tên món ăn" required error={fieldErrors?.name}>
          <TextInput
            id="cuisine-name"
            value={form.name}
            required
        ...
        <FormField htmlFor="cuisine-name-en" label="Tên tiếng Anh">
          <TextInput
            id="cuisine-name-en"
            value={form.nameEn}
            disabled={submitting}
        ...
        <FormField htmlFor="cuisine-avg-price" label="Giá tham khảo (VNĐ)" error={fieldErrors?.avgPrice}>
          <TextInput
            ...
            value={form.avgPrice}
            disabled={submitting}
            invalid={Boolean(fieldErrors?.avgPrice?.length)}
            placeholder="35000"
            onChange={(event) => updateField("avgPrice", event.target.value)}
          />
        </FormField>

        <FormField htmlFor="cuisine-description" label="Mô tả">
          <TextArea
        ...
        <FormField label="Địa danh liên quan" error={fieldErrors?.destinationIds}>
          <div className="flex max-h-40 flex-wrap gap-2 overflow-y-auto rounded-md border border-admin-line bg-admin-paper p-2.5">
            {destinations.length === 0 ? <span className="text-xs text-admin-muted">Chưa có địa danh nào</span> : null}

            {destinations.map((destination) => {
              const checked = form.destinationIds.includes(destination.id);
