...
function SummaryCard({
  ...
  return (
    ...
    >
      ...
      <p className="mt-2 font-display text-3xl font-semibold text-admin-ink">{value.toLocaleString("vi-VN")}</p>
    </button>
  );
}
...
export function AdminStoriesPage() {
  ...
  const { page, pageSize, hasPreviousPage, hasNextPage, nextPage, previousPage, resetPage } = usePagination({
    totalItems: data?.total ?? 0,
    pageSize: PAGE_SIZE,
  });

  const query = useDebounce(queryInput.trim(), 350, resetPage);
  ...
  const loadStories = useCallback(
    async (silent = false) => {
      ...
      try {
        ...
        setErrorMessage(error instanceof ApiRequestError ? error.message : "Không thể tải danh sách story");
      } finally {
        setLoading(false);
        setRefreshing(false);
      }
    },
    [locationId, page, pageSize, query, status],
  );
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      setErrorMessage(error instanceof ApiRequestError ? error.message : "Không thể xóa story");
    } finally {
      setDeleting(false);
    }
  }
  ...
  return (
    <div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {(Object.keys(STATUS_LABELS) as AdminStoryListStatus[]).map((item) => (
          <SummaryCard
            key={item}
      ...
      <section className="mt-5 overflow-hidden rounded-lg border border-admin-line bg-admin-paper-card">
        <div className="flex flex-col gap-3 border-b border-admin-line p-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="relative w-full max-w-lg">
            <Search size={17} className="absolute left-3 top-1/2 -translate-y-1/2 text-admin-muted" />
            <input
              value={queryInput}
        ...
        {loading ? (
          ...
          <div className="grid min-h-72 place-items-center px-6 text-center">
            <div>
              ...
              <p className="mt-3 font-medium text-admin-ink">Không có story phù hợp</p>
              <p className="mt-1 text-sm text-admin-muted">Hãy thử thay đổi từ khóa hoặc bộ lọc.</p>
            </div>
          </div>
        ) : (
                  ...
                  <tr key={story.id} className="hover:bg-white/45">
                    <td className="border-b border-admin-line px-4 py-3">
                      <div className="flex max-w-sm items-center gap-3">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="mt-1 line-clamp-2 text-xs leading-5 text-admin-muted">{story.content}</p>
                        </div>
                      </div>
                    </td>
                    <td className="border-b border-admin-line px-4 py-3">
                      <div className="flex items-center gap-2">
                        {story.author.avatarUrl ? (
                          <img src={story.author.avatarUrl} alt="" className="h-8 w-8 rounded-full object-cover" />
                        ) : (
                    ...
                    <td className="border-b border-admin-line px-4 py-3">
                      <div className="flex items-center gap-3 text-xs text-admin-muted">
                        ...
                        <span
                          className={`inline-flex items-center gap-1 ${story.reportCount ? "font-semibold text-admin-seal" : ""}`}
                        >
                          <Flag size={14} />
                          {story.reportCount}
                        </span>
      ...
      <Dialog
        ...
        description={
          detailStory
            ? `Đăng bởi ${detailStory.author.fullName || "Người dùng"} lúc ${formatVietnameseDateTime(detailStory.createdAt)}`
            : undefined
        }
        size="xl"
      >
        {detailStory ? (
          <div>
            {detailStory.coverImageUrl ? (
              <img src={detailStory.coverImageUrl} alt="" className="mb-5 max-h-72 w-full rounded-lg object-cover" />
            ) : null}
            <p className="whitespace-pre-wrap text-sm leading-7 text-admin-ink">{detailStory.content}</p>
            <div className="mt-5 flex flex-wrap gap-2 text-xs text-admin-muted">
              <span className="rounded-full bg-admin-paper px-3 py-1.5">
                {detailStory.locationName || "Chưa gắn khu vực"}
              </span>
      ...
      >
        ...
        />
        <p className="mt-1 text-right text-xs text-admin-muted">{deleteReason.length}/500</p>
      </Dialog>
    </div>
  );
}
