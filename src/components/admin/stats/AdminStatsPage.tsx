...
import { adminStatsApi, type AdminStatsData, type MonthlyComparison } from "@/src/lib/api-client/admin-stats";
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 text-xs font-medium ${
        positive ? "text-admin-moss" : negative ? "text-admin-seal" : "text-admin-muted"
      }`}
    >
      <Icon size={14} />
    ...
    <article className="rounded-xl border border-admin-line bg-admin-paper-card p-5">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-2 font-display text-4xl font-semibold text-admin-ink">{value.toLocaleString("vi-VN")}</p>
        </div>
...
function LineChart({
  ...
  const points = data.map((item, index) => ({
    ...item,
    x: padding.left + (data.length <= 1 ? 0 : (index / (data.length - 1)) * chartWidth),
  ...
  return (
    <div className="relative overflow-x-auto">
      {!hasData ? (
        <div className="absolute inset-0 grid place-items-center text-sm text-admin-muted">{emptyLabel}</div>
      ) : null}
      <svg
      ...
      >
        {[0, 0.25, 0.5, 0.75, 1].map((ratio) => {
          ...
          return (
            <g key={ratio}>
              <line x1={padding.left} y1={y} x2={width - padding.right} y2={y} stroke="#d9d3c4" strokeDasharray="4 5" />
              <text x={padding.left - 8} y={y + 4} textAnchor="end" fontSize="10" fill="#8a8575">
        ...
        {points.map((point, index) => (
          <g key={point.day}>
            ...
            {index === 0 || point.day % 5 === 0 || index === points.length - 1 ? (
              <text x={point.x} y={height - 10} textAnchor="middle" fontSize="10" fill="#8a8575">
                {point.day}
              </text>
            ) : null}
          </g>
...
function RankedBars({ rows }: { rows: Array<{ id: string; name: string; mentions: number }> }) {
      ...
      {rows.map((row, index) => (
        <div key={row.id}>
          <div className="mb-1.5 flex justify-between gap-3 text-sm">
            ...
            <span className="shrink-0 text-xs text-admin-muted">{row.mentions} lượt nhắc</span>
          </div>
          <div className="h-2 overflow-hidden rounded-full bg-admin-paper">
            <div className="h-full rounded-full bg-admin-gold" style={{ width: `${(row.mentions / max) * 100}%` }} />
          </div>
        </div>
      ))}
    </div>
  ...
  const loadStats = useCallback(
    async (silent = false) => {
      ...
      try {
        ...
        setErrorMessage(error instanceof ApiRequestError ? error.message : "Không thể tải dữ liệu thống kê");
      } finally {
        setLoading(false);
        setRefreshing(false);
      }
  ...
  return (
    <div>
      <AdminTopbar
        ...
        subtitle={
          data
            ? `Tổng quan ${monthLabel(data.month)} và so sánh với ${monthLabel(data.previousMonth)}`
            : "Theo dõi nội dung và mức độ tham gia của cộng đồng"
        }
        action={
          ...
          <div className="mt-5 grid gap-5 xl:grid-cols-2">
            <section className="rounded-xl border border-admin-line bg-admin-paper-card p-5">
              <div className="mb-4">
                ...
                <p className="mt-1 text-xs text-admin-muted">
                  {data.monthly.users.current} tài khoản mới trong {monthLabel(data.month)}
                </p>
              </div>
              <LineChart
                data={data.timelines.users}
                color="#4f6d5a"
                emptyLabel="Chưa có tài khoản mới trong tháng này"
              />
            </section>
            ...
            <section className="rounded-xl border border-admin-line bg-admin-paper-card p-5">
              <div className="mb-4">
                ...
                <p className="mt-1 text-xs text-admin-muted">
                  {data.monthly.stories.current} story được đăng trong {monthLabel(data.month)}
                </p>
              </div>
          ...
          <div className="mt-5 grid gap-5 xl:grid-cols-[1fr_1fr_0.85fr]">
            ...
            <section className="rounded-xl border border-admin-line bg-admin-paper-card p-5">
              ...
              {!data.mostReportedStories.length ? (
                <div className="grid min-h-56 place-items-center text-sm text-admin-muted">
                  Không có báo cáo Story trong tháng này.
                </div>
              ) : (
                <ol className="mt-5 divide-y divide-admin-line">
                  {data.mostReportedStories.map((story, index) => (
                    <li key={story.id} className="flex items-center gap-3 py-3">
                      <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-admin-seal-light text-xs font-semibold text-admin-seal">
                        {index + 1}
                      </span>
              ...
              <div className="mt-7 flex h-4 overflow-hidden rounded-full bg-admin-paper">
                {statusValues.map((item) => (
                  <div
                    ...
                    style={{
                      width: `${statusTotal ? (item.value / statusTotal) * 100 : 0}%`,
                    }}
