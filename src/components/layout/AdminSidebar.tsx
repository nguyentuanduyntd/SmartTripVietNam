...
import { BarChart3, BookImage, MapPin, Route, ShieldAlert, Utensils } from "lucide-react";
  ...
  return (
    <aside className="w-60 flex-shrink-0 bg-admin-indigo px-4 py-7 text-[#C8CDD3]">
      <div className="mb-6 flex items-center gap-2.5 border-b border-white/10 pb-5">
        ...
        <div>
          <p className="font-display text-sm font-medium text-[#F4F2EA]">SmartTrip</p>
      ...
      {NAV_GROUPS.map((group) => (
        <div key={group.label} className="mb-5">
          <p className="mb-2 ml-2.5 text-[11px] font-medium uppercase tracking-wide text-[#7C8894]">{group.label}</p>
