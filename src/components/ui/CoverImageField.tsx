...
export type CoverImageController = {
  ...
  handleCoverChange: (event: ChangeEvent<HTMLInputElement>) => void;
  handleRemoveCover: () => void;
};

export function useCoverImageField(initialImageUrl?: string | null): CoverImageController {
  const initialImageUrlRef = useRef(initialImageUrl ?? null);
  ...
  const handleRemoveCover = useCallback(() => {
    ...
    setPreviewUrl(null);
    setImageError(null);
    setRemoveCover(Boolean(initialImageUrlRef.current));
  }, [revokeObjectUrl]);
...
export function CoverImageField({
  ...
  const { coverFile, previewUrl, imageError, handleCoverChange, handleRemoveCover } = controller;

  return (
    <FormField label={label} error={imageError}>
      {previewUrl ? (
        <div className="relative overflow-hidden rounded-lg border border-admin-line bg-admin-paper">
          {}
          {}
          <img src={previewUrl} alt={imageAlt} className="h-48 w-full object-cover" />
        ...
        <label
          ...
          className={`flex h-40 flex-col items-center justify-center rounded-lg border border-dashed border-admin-line bg-admin-paper text-admin-muted transition ${
            disabled ? "cursor-not-allowed opacity-60" : "cursor-pointer hover:border-admin-gold hover:text-admin-ink"
          }`}
        >
          <ImagePlus size={28} />
          ...
          <span className="mt-1 text-xs">JPEG, PNG, WebP, AVIF — tối đa 5MB</span>
        </label>
      )}
      ...
      {previewUrl ? (
        <label
          ...
          className={`mt-2 inline-flex items-center gap-1.5 text-sm font-medium text-admin-gold ${
            disabled ? "cursor-not-allowed opacity-60" : "cursor-pointer hover:underline"
          }`}
        >
          <ImagePlus size={15} />
          Chọn ảnh khác
      ...
      {coverFile ? <p className="mt-1 break-all text-xs text-admin-muted">Đã chọn: {coverFile.name}</p> : null}
    </FormField>
  );
}
