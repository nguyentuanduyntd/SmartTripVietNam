      ...
      >
        <header className="flex items-start justify-between gap-4 border-b border-admin-line px-6 py-5">
          <div className="min-w-0">
            <h2 id={titleId} className="font-display text-2xl font-semibold text-admin-ink">
              {title}
            </h2>

            {description ? (
              <div id={descriptionId} className="mt-1.5 text-sm leading-6 text-admin-muted">
                {description}
              </div>
            ) : null}
          </div>
        ...
        {footer ? (
          <footer className="flex justify-end gap-2 border-t border-admin-line px-6 py-4">{footer}</footer>
        ) : null}
      </section>
    </div>
  );
