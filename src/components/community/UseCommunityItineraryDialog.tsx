...
import { CalendarDays, Copy, Loader2, UsersRound, X } from "lucide-react";
...
export function UseCommunityItineraryDialog({
  ...
  const [adultCount, setAdultCount] = useState(Math.max(defaultAdultCount, 1));

  const [childCount, setChildCount] = useState(Math.max(defaultChildCount, 0));

  const [roomCount, setRoomCount] = useState(Math.max(defaultRoomCount, 1));
  ...
  async function submit() {
    ...
    try {
      const response = await fetch(`/api/community/posts/${postId}/use-itinerary`, {
        method: "POST",
