...
import type { CommunityImage } from "@/src/components/community/community-types";
...
export function CommunityImageCarousel({ images, title, variant = "card" }: CommunityImageCarouselProps) {
    ...
    return (
      <div
        className={`grid place-items-center bg-[radial-gradient(circle_at_top_right,#f6d8cf,transparent_42%),linear-gradient(135deg,#dcebe7,#f8f0e4)] ${
          variant === "detail" ? "min-h-[320px] sm:min-h-[480px]" : "h-64 sm:h-72"
        }`}
      >
        <div className="text-center text-[#557b76]">
          <Images size={38} className="mx-auto" />

          <p className="mt-3 text-sm font-bold">Chưa có ảnh trải nghiệm</p>
        </div>
      </div>
    );
  }
            ...
            {images.map((image, index) => (
              <button
                ...
                className={`h-1.5 rounded-full transition-all ${
                  index === safeIndex ? "w-5 bg-white" : "w-1.5 bg-white/60"
                }`}
              />
            ))}
          </div>
