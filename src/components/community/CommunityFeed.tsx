...
import { Bookmark, Clock3, Heart, Loader2, MapPin, MessageCircle, Route, Star, WalletCards } from "lucide-react";
...
import { formatOptionalVnd as formatCurrency, formatRelativeTime } from "@/src/lib/formatters";
...
function CommunityStars({ value }: { value: number | null }) {
  const rating = value ?? 0;

  return (
    <span className="inline-flex items-center gap-0.5" aria-label={`${rating}/5 sao`}>
      {Array.from({ length: 5 }, (_, index) => (
        <Star key={index} size={14} className={index < rating ? "fill-[#e4a32f] text-[#e4a32f]" : "text-[#d8d0c5]"} />
      ))}
    </span>
  );
}

export function CommunityFeed({ locations, isAuthenticated }: CommunityFeedProps) {
  ...
  const SORT_OPTIONS: Array<{
    ...
    { value: "saved", label: t("sortSaved") },
  ];

  const [sort, setSort] = useState<FeedSort>("latest");
  const [locationId, setLocationId] = useState("");
  const [items, setItems] = useState<CommunityPostCardData[]>([]);
  ...
  const loadFeed = useCallback(
    async (targetPage: number, replace: boolean) => {
      ...
      try {
        ...
        const payload = await readCommunityApi<CommunityFeedData>(response);

        if (response.status === 401 && sort === "saved") {
        ...
        console.error("[COMMUNITY FEED ERROR]", loadError);

        setError(loadError instanceof Error ? loadError.message : t("fallbackError"));
      } finally {
        if (version === requestVersion.current) {
          setIsLoading(false);
  ...
  function requireLogin() {
    ...
    window.location.assign(`/auth/login?next=${encodeURIComponent("/community")}`);

    return false;
  }

  async function toggleLike(post: CommunityPostCardData) {
    if (!requireLogin()) {
      return;
    }
    ...
    try {
      ...
      const payload = await readCommunityApi<{
        postId: string;
        liked: boolean;
        likeCount: number;
      }>(response);
      ...
      console.error("[COMMUNITY LIKE ERROR]", toggleError);

      void loadFeed(1, true);
    }
  }

  async function toggleSave(post: CommunityPostCardData) {
    if (!requireLogin()) {
      return;
    }
      ...
      current.map((item) =>
        item.id === post.id
          ? {
              ...item,
              savedByMe: next,
              saveCount: Math.max(0, item.saveCount + (next ? 1 : -1)),
            }
          : item,
      ),
    );
    ...
    try {
      ...
      if (!response.ok || !payload.success || !payload.data) {
        throw new Error(payload.message ?? "Không thể cập nhật bài đã lưu.");
      }
      ...
      console.error("[COMMUNITY SAVE ERROR]", toggleError);
      ...
      <div className="rounded-[28px] border border-white/80 bg-[#fffaf1] p-4 shadow-[0_14px_45px_rgba(23,58,59,0.06)] sm:p-5">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div className="flex flex-wrap gap-2">
            {SORT_OPTIONS.map((option) => (
              <button
                ...
                type="button"
                onClick={() => {
                  if (option.value === "saved" && !isAuthenticated) {
                    window.location.href = `/auth/login?next=${encodeURIComponent("/community")}`;
                    return;
                  }
          ...
          <div className="flex min-w-0 items-center gap-2">
            <MapPin size={16} className="shrink-0 text-[#d85b48]" />
      ...
      {isLoading ? (
        <div className="mt-8 grid min-h-64 place-items-center rounded-[30px] border border-white/80 bg-[#fffaf1]">
          <div className="text-center">
            <Loader2 size={30} className="mx-auto animate-spin text-[#34706b]" />
            <p className="mt-3 text-sm font-bold text-[#647773]">{t("loading")}</p>
          </div>
        </div>
      ) : items.length === 0 ? (
        <div className="mt-8 rounded-[30px] border border-dashed border-[#cdbfae] bg-[#fffaf1] px-6 py-16 text-center">
          <Route size={36} className="mx-auto text-[#5c817b]" />
          <h2 className="mt-4 font-display text-3xl font-semibold">{t("emptyTitle")}</h2>
        ...
        <>
          <div className="mt-7 grid gap-6 lg:grid-cols-2 xl:grid-cols-3">
            {items.map((post) => {
              const title = post.title ?? t("memberSmartTrip");

              const estimatedCost = formatCurrency(post.estimatedCost);
              ...
              return (
                ...
                >
                  <CommunityImageCarousel images={post.images} title={title} />
                  ...
                  <div className="p-5 sm:p-6">
                    <div className="flex items-center gap-3">
                      {post.author.avatarUrl ? (
                        <img
                          ...
                          alt={post.author.fullName ?? t("member")}
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-sm font-extrabold">
                          {post.author.fullName ?? t("memberSmartTrip")}
                        </p>
                      ...
                      <CommunityStars value={post.rating} />
                    </div>

                    <Link href={`/community/${post.id}`} className="group/title block">
                    ...
                    <p className="mt-3 line-clamp-3 text-sm leading-6 text-[#687773]">{post.content}</p>
                    ...
                    {post.destinations.length > 0 ? (
                      <p className="mt-4 line-clamp-1 text-xs font-semibold text-[#7a8682]">
                        ...
                        {post.destinations
                          ...
                          .join(" · ")}
                        {post.destinations.length > 3 ? ` · +${post.destinations.length - 3}` : ""}
                      </p>
                    ) : null}
                      ...
                      <div className="flex items-center gap-1">
                        <button
                          ...
                          className={`inline-flex items-center gap-1.5 rounded-full px-3 py-2 text-xs font-extrabold transition ${
                            post.likedByMe ? "bg-[#fff0ed] text-[#cf5545]" : "text-[#65736f] hover:bg-[#f4eee5]"
                          }`}
                        >
                          <Heart size={17} className={post.likedByMe ? "fill-current" : ""} />
                          {post.likeCount}
                        </button>
                        ...
                        <button
                          ...
                          className={`inline-flex items-center gap-1.5 rounded-full px-3 py-2 text-xs font-extrabold transition ${
                            post.savedByMe ? "bg-[#edf6f3] text-[#34706b]" : "text-[#65736f] hover:bg-[#f4eee5]"
                          }`}
                        >
                          <Bookmark size={17} className={post.savedByMe ? "fill-current" : ""} />
                          {post.saveCount}
                        </button>
                      </div>
          ...
          {hasMore ? (
            <div className="mt-8 text-center">
              <button
                ...
                onClick={() => void loadFeed(page + 1, false)}
              ...
              >
                ...
                {isLoadingMore ? t("loading2") : t("loadMore")}
              </button>
            </div>
          ) : null}
        </>
