...
import { formatOptionalVnd, formatVietnameseDate, formatVietnameseDateTime } from "@/src/lib/formatters";
...
function formatDate(value: string | null) {
  ...
  return formatVietnameseDate(value, value);
}
      ...
      {Array.from({ length: 5 }, (_, index) => {
        ...
        const icon = (
          <Star
            ...
            className={star <= value ? "fill-[#e4a32f] text-[#e4a32f]" : "text-[#d8d0c5]"}
          />
        );
...
export function CommunityPostDetail({ postId, currentUserId }: CommunityPostDetailProps) {
  ...
  const loadPost = useCallback(async () => {
    ...
    const payload = await readCommunityApi<CommunityPostDetailData>(response);

    if (!response.ok || !payload.success || !payload.data) {
      throw new Error(payload.message ?? "Không tìm thấy bài chia sẻ.");
    }

    setPost(payload.data);
    setEditTitle(payload.data.title ?? "");
    setEditContent(payload.data.content);
    setEditRating(payload.data.rating ?? 5);
  }, [postId]);
  ...
  const loadComments = useCallback(async () => {
    ...
    const payload = await readCommunityApi<CommunityCommentThread[]>(response);

    if (!response.ok || !payload.success || !payload.data) {
      throw new Error(payload.message ?? "Không thể tải bình luận.");
    }
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        console.error("[COMMUNITY DETAIL ERROR]", loadError);

        setError(loadError instanceof Error ? loadError.message : "Không thể tải bài chia sẻ.");
      } finally {
        if (alive) {
          setIsLoading(false);
        }
  ...
  const isOwner = Boolean(currentUserId && post && currentUserId === post.userId);

  const estimatedCost = useMemo(() => formatCurrency(post?.estimatedCost ?? null), [post?.estimatedCost]);
  ...
  function requireLogin() {
    ...
    window.location.href = `/auth/login?next=${encodeURIComponent(`/community/${postId}`)}`;
  ...
  return (
    <article className="mt-7">
      <section className="overflow-hidden rounded-[32px] border border-white/80 bg-[#fffaf1] shadow-[0_22px_70px_rgba(23,58,59,0.09)]">
        <CommunityImageCarousel images={post.images} title={title} variant="detail" />
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            ...
            {post.tripStartDate ? (
              <span className="inline-flex items-center gap-1.5 rounded-full bg-[#f5efe6] px-3 py-2 text-xs font-bold text-[#697773]">
                ...
                {formatDate(post.tripStartDate)}
                {post.tripEndDate ? ` – ${formatDate(post.tripEndDate)}` : ""}
              </span>
            ) : null}
            ...
            {snapshot.days.map((day) => (
              ...
              >
                <summary className="flex cursor-pointer list-none items-center gap-3 px-5 py-4">
                  ...
                  <ChevronRight size={18} className="text-[#6d7b77] transition group-open:rotate-90" />
                </summary>

                <div className="border-t border-[#eee5da] px-5 py-5">
                  {day.description ? <p className="mb-4 text-sm leading-6 text-[#6d7a76]">{day.description}</p> : null}
                    ...
                    {day.items.map((item, index) => (
                      ...
                      >
                        <div className="flex items-start gap-3">
                          <Route size={16} className="mt-0.5 shrink-0 text-[#34706b]" />

                          <div>
                            <p className="text-sm font-extrabold">{item.title}</p>

                            {item.destinationName ? (
                              <p className="mt-1 text-xs font-bold text-[#52736d]">{item.destinationName}</p>
                            ) : null}
      ...
      >
        ...
        <div className="mt-4 rounded-[22px] border border-[#ded3c5] bg-white p-4">
          ...
          <div className="mt-3 flex items-center justify-between gap-3">
            <span className="text-xs text-[#88928f]">{commentText.length}/1500</span>
