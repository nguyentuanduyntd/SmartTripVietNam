...
function buildGoogleMapsUrl(restaurant: RestaurantDiscoveryItem) {
...
function getPrimaryCuisine(restaurant: RestaurantDiscoveryItem) {
  return restaurant.cuisines.find((cuisine) => cuisine.isSignature) ?? restaurant.cuisines[0];
}

function getDishPrice(restaurant: RestaurantDiscoveryItem) {
  const cuisinePrice = getPrimaryCuisine(restaurant)?.avgPrice;

  if (cuisinePrice !== undefined && cuisinePrice !== null) {
    return cuisinePrice;
  }

  const { priceMin, priceMax } = restaurant;

  if (priceMin != null && priceMax != null) {
    return Math.round((priceMin + priceMax) / 2_000) * 1_000;
  }
...
function RestaurantCard({
  ...
  const primaryCuisine = getPrimaryCuisine(restaurant);
  const dishName = primaryCuisine?.name ?? "Ẩm thực địa phương";
  const dishPrice = getDishPrice(restaurant);
        ...
        {restaurant.rating != null ? (
          <div className="absolute right-4 top-4 rounded-2xl bg-white/92 px-3 py-2 text-right text-[#173a3b] shadow-sm backdrop-blur">
            <p className="inline-flex items-center gap-1 text-sm font-extrabold">
              <Star size={13} fill="currentColor" className="text-[#d99b2b]" />
              {restaurant.rating.toFixed(1)}
            </p>
            <p className="mt-0.5 text-[9px] font-bold text-[#71817d]">{restaurant.reviewCount ?? 0} đánh giá</p>
          </div>
        ) : null}
      </div>

      <div className="flex flex-1 flex-col p-5 sm:p-6">
        <h3 className="font-display text-2xl font-semibold leading-tight text-[#173a3b]">{dishName}</h3>

        <p className="mt-3 flex items-start gap-2 text-sm leading-6 text-[#6f7e7a]">
          <MapPin size={15} aria-hidden="true" className="mt-1 shrink-0 text-[#e05e4c]" />
        ...
        <div className="mt-auto border-t border-[#eee6da] pt-5">
          <div className="flex items-end justify-between gap-3">
            <div className="min-w-0">
              <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#929c99]">Giá tham khảo / người</p>
              <p className="mt-1 truncate text-lg font-extrabold text-[#d85b48]">{formatMoney(dishPrice)}</p>
            </div>
...
export function FoodDiscoveryPanel() {
  ...
  const [result, setResult] = useState<RestaurantDiscoveryResult | null>(null);
  ...
  useEffect(() => {
    ...
    async function loadRestaurants(selectedLocation: ActiveLocation) {
      ...
      try {
        ...
        const payload = (await response.json()) as ApiPayload<RestaurantDiscoveryResult>;

        if (!response.ok || !payload.success || !payload.data) {
          throw new Error(payload.message ?? "Chưa thể tìm quán ăn lúc này.");
        }
        ...
        console.error("[FOOD DISCOVERY ERROR]", loadError);

        setResult(null);
        setError(loadError instanceof Error ? loadError.message : "Chưa thể tìm quán ăn lúc này.");
      } finally {
  ...
  }, [activeLocation, debouncedSearch, sort]);
  ...
  function handleMapLocationSelect(latitude: number, longitude: number) {
    setError(null);
  ...
  const hasFilters = Boolean(search || sort !== "best_match");
          ...
          <div className="relative grid gap-6 p-6 sm:p-8 lg:grid-cols-[0.72fr_1.28fr] lg:items-center lg:p-10">
            <div className="lg:pr-4">
              <h2 className="max-w-3xl font-display text-4xl font-semibold leading-[1.02] tracking-[-0.035em] sm:text-5xl">
                Ăn gì gần bạn,
                <span className="block italic text-[#f5d99c]">ngay lúc này?</span>
              </h2>

              <p className="mt-5 max-w-md text-sm leading-6 text-white/68 sm:text-base sm:leading-7">
                Bật GPS để tìm món ăn và quán gần vị trí hiện tại. Bạn vẫn có thể chạm vào bản đồ để tìm quanh một điểm
                khác.
              </p>
              ...
              <div className="mt-7">
                ...
                >
                  {isLocating ? <Loader2 size={15} className="animate-spin" /> : <LocateFixed size={15} />}
                  GPS thiết bị
                </button>
              </div>

              <p className="mt-4 text-xs font-semibold text-white/55" aria-live="polite">
                {isLocating
