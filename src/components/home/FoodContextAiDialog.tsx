...
import { ArrowRight, Bot, Loader2, MapPin, Plus, Send, Sparkles, X } from "lucide-react";
...
function buildMapsUrl(restaurant: FoodAiRestaurantContext) {
...
export function FoodContextAiDialog({
  ...
  const [lastResult, setLastResult] = useState<FoodContextResponse | null>(null);
  ...
  async function ask(overrideQuestion?: string) {
    ...
    if (!question || isLoading || !restaurant) {
      return;
    }
    ...
    try {
      const response = await fetch("/api/ai/food/context", {
        method: "POST",
        headers: {
      ...
      const payload = (await response.json()) as ApiPayload<FoodContextResponse>;

      if (!response.ok || !payload.success || !payload.data) {
        throw new Error(payload.message ?? "SmartTrip AI chưa thể trả lời lúc này.");
      }
      ...
      console.error("[FOOD CONTEXT AI UI ERROR]", askError);
      setError(askError instanceof Error ? askError.message : "SmartTrip AI chưa thể trả lời lúc này.");
    } finally {
      setIsLoading(false);
    }
  }
  ...
  function addToPlanner() {
    ...
    const fallbackPrompt = [
      ...
      currentRestaurant.address ? `Địa chỉ: ${currentRestaurant.address}.` : null,
      currentRestaurant.cuisines.length
        ? `Món liên quan: ${currentRestaurant.cuisines.map((item) => item.name).join(", ")}.`
        : null,
    ...
    const prompt = lastResult?.plannerPrompt ?? fallbackPrompt;

    window.location.href = `/planner/ai?foodPrompt=${encodeURIComponent(prompt)}`;
  }
  ...
  return (
    ...
    >
      <div className="flex max-h-[92dvh] w-full max-w-3xl flex-col overflow-hidden rounded-t-[30px] border border-white/80 bg-[#fffaf1] shadow-[0_30px_100px_rgba(12,40,40,.3)] sm:max-h-[82dvh] sm:rounded-[32px]">
        ...
        <div ref={scrollRef} className="min-h-0 flex-1 overflow-y-auto bg-[#f7f2ea] px-4 py-5 sm:px-6">
          {messages.length === 0 ? (
            <div className="rounded-[24px] border border-[#dce7e2] bg-white px-5 py-5">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-extrabold text-[#315f5f]">Hỏi mình về quán này</p>
                  <p className="mt-1 text-sm leading-6 text-[#667873]">
                    Mình chỉ dùng dữ liệu SmartTrip của quán và các món liên quan. Nếu dữ liệu chưa đủ, mình sẽ nói rõ
                    thay vì đoán.
                  </p>
                </div>
              </div>
          ...
          <div className="space-y-4">
            {messages.map((message, index) =>
              message.role === "user" ? (
                ...
                <div key={`${message.role}-${index}`} className="flex items-start gap-2.5">
          ...
          {lastResult?.quickReplies?.length ? (
            <div className="mt-4 flex flex-wrap gap-2 pl-10">
              {lastResult.quickReplies.map((item) => (
                <button
                  key={item.label}
                  type="button"
                  disabled={isLoading}
          ...
          {lastResult?.disclaimer ? (
            <p className="mt-4 text-[11px] leading-5 text-[#8a9491]">{lastResult.disclaimer}</p>
          ) : null}
        </div>
