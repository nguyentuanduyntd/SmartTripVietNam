...
import { getLocalizedHomeStaticData } from "@/src/i18n/home-static-data";
...
function SectionHeading({
  ...
  return (
    <div className={align === "center" ? "mx-auto max-w-3xl text-center" : "max-w-3xl"}>
      <p className="mb-3 text-xs font-extrabold uppercase tracking-[0.22em] text-[#e55c49]">{eyebrow}</p>

      <h2 className="font-display text-4xl font-semibold leading-[1.03] tracking-[-0.035em] text-[#173a3b] sm:text-5xl lg:text-6xl">
        {title}
      </h2>

      {description ? <p className="mt-5 text-base leading-8 text-[#60706d] sm:text-lg">{description}</p> : null}
    </div>
  );
}
...
export function CityEditorialSection({ items }: CityEditorialSectionProps) {
  const t = useTranslations("Home.sections.editorial");

  const locale = useResolvedLocale();

  const fallbackData = useMemo(() => getLocalizedHomeStaticData(locale), [locale]);

  const sectionItems = items ?? fallbackData.cityEditorial;
    ...
    >
      ...
      <div className="relative mx-auto max-w-[1440px]">
        <div className="grid gap-8 lg:grid-cols-[0.78fr_1.22fr] lg:items-end">
          <div>
            <p className="text-xs font-extrabold uppercase tracking-[0.22em] text-[#f5ba54]">{t("eyebrow")}</p>

            <h2 className="mt-4 font-display text-5xl font-semibold leading-[0.98] tracking-[-0.04em] sm:text-6xl lg:text-7xl">
              {t("titleLine1")}

              <span className="block italic text-[#f7dca5]">{t("titleLine2")}</span>
            </h2>
          </div>

          <p className="max-w-2xl text-lg leading-8 text-white/68 lg:justify-self-end">{t("description")}</p>
        </div>
            ...
            return (
              ...
              >
                ...
                <div className="absolute inset-x-0 bottom-0 p-7 sm:p-8">
                  ...
                  <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#f4c56e]">{item.kicker}</p>

                  <h3 className="mt-2 font-display text-4xl font-semibold">{item.city}</h3>

                  <p className="mt-3 leading-7 text-white/72">{item.title}</p>
                </div>
              </article>
            );
          })}
...
export function FeaturedDestinationsSection({ items }: FeaturedDestinationsSectionProps) {
  const t = useTranslations("Home.sections.destinations");

  const locale = useResolvedLocale();

  const fallbackData = useMemo(() => getLocalizedHomeStaticData(locale), [locale]);

  const sectionItems = items ?? fallbackData.featuredDestinations;
  ...
  const filters: Array<{
    ...
    {
      id: "da-nang",
      label: t("filters.daNang"),
    },
    {
      id: "hoi-an",
      label: t("filters.hoiAn"),
    },
  ];

  const visibleItems = useMemo(
    () => (filter === "all" ? sectionItems : sectionItems.filter((item) => item.city === filter)),
    [filter, sectionItems],
  );

  return (
    <section id="diem-den" className="bg-[#f7f0e4] px-5 py-24 sm:px-8 lg:px-12 lg:py-32">
          ...
          {visibleItems.map((item, index) => (
            <Link
              ...
              className={`group overflow-hidden rounded-[30px] border border-[#ded3c3] bg-[#fffaf1] shadow-[0_16px_55px_rgba(35,55,50,0.06)] transition-all hover:-translate-y-2 hover:shadow-[0_24px_65px_rgba(35,55,50,0.13)] ${
                index === 0 && filter === "all" ? "md:col-span-2 xl:col-span-1" : ""
              }`}
            >
              ...
              <div className="p-6 sm:p-7">
                ...
                <div className="mt-3 flex items-start justify-between gap-4">
                  <div>
                    <h3 className="font-display text-3xl font-semibold text-[#173a3b]">{item.name}</h3>

                    <p className="mt-3 leading-7 text-[#667370]">{item.description}</p>
                  </div>
...
export function JourneySection({ items }: JourneySectionProps) {
  const t = useTranslations("Home.sections.journey");

  const locale = useResolvedLocale();

  const fallbackData = useMemo(() => getLocalizedHomeStaticData(locale), [locale]);

  const sectionItems = items ?? fallbackData.journeys;
  ...
  return (
    <section id="hanh-trinh" className="bg-[#efe6d7] px-5 py-24 sm:px-8 lg:px-12 lg:py-32">
      <div className="mx-auto max-w-[1440px]">
          ...
          {sectionItems.map((item, index) => (
            ...
            >
              ...
              <div className="flex flex-1 flex-col p-7">
                <h3 className="font-display text-3xl font-semibold text-[#173a3b]">{item.title}</h3>

                <p className="mt-3 line-clamp-3 leading-7 text-[#687572]">{item.description}</p>
                ...
                <div className="mt-6 space-y-3">
                  <div className="flex items-center gap-3 rounded-2xl bg-[#f3ece1] px-4 py-3">
                    ...
                    <div className="min-w-0">
                      <p className="text-xs font-semibold text-[#78837f]">{t("departure")}</p>

                      <p className="truncate text-sm font-bold text-[#315f5f]">{item.startLocation}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3 rounded-2xl bg-[#f3ece1] px-4 py-3">
                    ...
                    <div className="min-w-0">
                      <p className="text-xs font-semibold text-[#78837f]">{t("duration")}</p>

                      <p className="truncate text-sm font-bold text-[#315f5f]">{item.duration}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3 rounded-2xl bg-[#f3ece1] px-4 py-3">
                    ...
                    <div className="min-w-0">
                      <p className="text-xs font-semibold text-[#78837f]">{t("estimatedPrice")}</p>

                      <p className="truncate text-sm font-bold text-[#315f5f]">{item.price}</p>
                    </div>
                  </div>
                </div>

                <Link href={item.href} className="mt-auto inline-flex items-center gap-2 pt-7 font-bold text-[#e25d49]">
                  {t("viewJourney")}
...
export function ExperienceSection() {
  const t = useTranslations("Home.sections.experience");

  const localizedFeatures = BRAND_FEATURES.map((feature, index) => {
    let label: string = feature.label;

    switch (index) {
      case 0:
        label = t("features.personalized");
        break;

      case 1:
        label = t("features.visual");
        break;

      case 2:
        label = t("features.localData");
        break;
  ...
  const localizedStories = STORIES.map((story, index) => {
    switch (index) {
      case 0:
        return {
          ...story,

          title: t("stories.lantern.title"),

          location: t("stories.lantern.location"),

          imageAlt: t("stories.lantern.imageAlt"),
        };
      ...
      case 1:
        return {
          ...
          title: t("stories.hueMorning.title"),

          location: t("stories.hueMorning.location"),

          imageAlt: t("stories.hueMorning.imageAlt"),
        };
      ...
      case 2:
        return {
          ...
          title: t("stories.beach.title"),

          location: t("stories.beach.location"),

          imageAlt: t("stories.beach.imageAlt"),
        };
  ...
  return (
    <section id="trai-nghiem" className="bg-[#fffaf1] px-5 py-24 sm:px-8 lg:px-12 lg:py-32">
      <div className="mx-auto max-w-[1440px]">
        <div className="grid gap-10 lg:grid-cols-[0.78fr_1.22fr] lg:items-end">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:justify-self-end">
            {localizedFeatures.map((feature) => {
              ...
              return (
                <div key={feature.label} className="rounded-2xl bg-[#f2eadc] p-4 text-center text-[#315f5f]">
                  <Icon className="mx-auto" size={20} />

                  <p className="mt-2 text-xs font-bold leading-5">{feature.label}</p>
                </div>
              );
            })}
          </div>
          ...
          {localizedStories.map((story, index) => (
            <article
              ...
              className={`group relative overflow-hidden rounded-[34px] ${
                index === 0 ? "min-h-[610px] lg:row-span-2" : "min-h-[292px]"
              }`}
            >
              <CloudinaryVisual
                source={story.image}
                alt={story.imageAlt}
                imageOptions={{
                  width: index === 0 ? 1100 : 850,

                  height: index === 0 ? 1200 : 600,
                }}
                className="absolute inset-0 transition-transform duration-700 group-hover:scale-105"
              />
              ...
              <div className="absolute inset-x-0 bottom-0 p-7 text-white sm:p-9">
                ...
                <h3 className={`mt-3 font-display font-semibold ${index === 0 ? "text-4xl sm:text-5xl" : "text-3xl"}`}>
                  {story.title}
                </h3>
...
export function PlannerSection() {
  const t = useTranslations("Home.sections.planner");

  const locale = useResolvedLocale();

  const localizedCities = useMemo(() => getLocalizedHomeStaticData(locale).cities, [locale]);
          ...
          <div className="rounded-[30px] border border-white/14 bg-white/8 p-4 backdrop-blur sm:p-5">
            <div className="rounded-[24px] bg-[#fffaf1] p-5 text-[#173a3b] sm:p-6">
              <div className="flex items-center gap-3 border-b border-[#ddd1bf] pb-4">
                ...
                <div>
                  <p className="font-bold">{t("assistantName")}</p>

                  <p className="text-xs text-[#74807d]">{t("assistantStatus")}</p>
                </div>
              ...
              <div className="mt-4 rounded-2xl border border-[#dcd1c0] p-4">
                ...
                <div className="mt-4 flex flex-wrap gap-2 text-xs font-semibold text-[#315f5f]">
                  {localizedCities.map((city) => (
                    <span key={city.id} className="rounded-full bg-[#e5efeb] px-3 py-1.5">
                      {city.name}
                    </span>
                  ))}
                </div>
