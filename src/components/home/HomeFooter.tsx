...
export function HomeFooter() {
  ...
  const navigationLinks = [
    ...
    {
      href: "/food",
      label: t("navigation.cuisine"),
    },
    {
      href: "/#hanh-trinh",
      label: t("navigation.journeys"),
    },
  ];
    ...
    <footer className="bg-[#102f30] px-5 pb-8 pt-16 text-white sm:px-8 lg:px-12 lg:pt-20">
      <div className="mx-auto max-w-[1440px]">
        <div className="grid gap-12 border-b border-white/12 pb-14 md:grid-cols-2 lg:grid-cols-[1.2fr_0.8fr_0.8fr_1fr]">
          <div>
            <Link href="/" aria-label={tHeader("brandHomeAria")} className="inline-flex items-center gap-3">
              <span className="grid h-12 w-12 place-items-center rounded-2xl bg-[#f25f4b]">
                <Landmark size={24} />
              </span>

              <span className="font-display text-3xl font-semibold">{tHeader("brand")}</span>
            </Link>

            <p className="mt-5 max-w-sm leading-7 text-white/60">{t("description")}</p>
          ...
          <div>
            <h3 className="text-sm font-extrabold uppercase tracking-[0.18em] text-[#f4c66f]">{t("exploreTitle")}</h3>

            <div className="mt-5 grid gap-3">
              {navigationLinks.map((item) => (
                <Link key={item.href} href={item.href} className="text-white/65 transition-colors hover:text-white">
                  {item.label}
                </Link>
              ))}
            </div>
          ...
          <div>
            <h3 className="text-sm font-extrabold uppercase tracking-[0.18em] text-[#f4c66f]">{t("supportTitle")}</h3>
          ...
          <div>
            <h3 className="text-sm font-extrabold uppercase tracking-[0.18em] text-[#f4c66f]">{t("contactTitle")}</h3>
            ...
            <div className="mt-5 grid gap-4 text-sm leading-6 text-white/65">
              ...
              <a href="mailto:hello@rucromientrung.vn" className="flex items-center gap-3 hover:text-white">
                <Mail size={17} />
        ...
        <div className="flex flex-col gap-3 pt-7 text-sm text-white/42 sm:flex-row sm:items-center sm:justify-between">
          <p>© 2026 {tHeader("brand")}. SmartTripVietNam.</p>
