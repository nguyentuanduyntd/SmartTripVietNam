...
import type { PlannerConversationState } from "@/src/components/planner/ai/chat/ai-travel-chat.types";
import { formatOptionalVnd } from "@/src/lib/formatters";

function createCompleteState(overrides: Partial<PlannerConversationState> = {}): PlannerConversationState {
  return {
    ...createInitialConversationState(),
...
describe("travel-chat.utils", () => {
  ...
  describe("createChatId", () => {
    it("tạo id bằng crypto.randomUUID khi trình duyệt hỗ trợ", () => {
      ...
      expect(createChatId()).toBe("msg-11111111-1111-4111-8111-111111111111");
      expect(createChatId("assistant")).toBe("assistant-11111111-1111-4111-8111-111111111111");
  ...
  describe("isConversationReady", () => {
    it("trả true khi có đủ thông tin bắt buộc", () => {
      expect(isConversationReady(createCompleteState())).toBe(true);
    });
    ...
    it.each([
      ...
      expect(isConversationReady(createCompleteState(overrides))).toBe(false);
    });
  });

  describe("buildPlannerRequestFromConversation", () => {
    it("trả null khi hội thoại chưa đủ dữ liệu", () => {
      expect(buildPlannerRequestFromConversation(createInitialConversationState())).toBeNull();
    });
  ...
  describe("findLocationLabel", () => {
    ...
    it("tìm tên từ danh sách bằng locationId", () => {
      expect(findLocationLabel(createCompleteState(), locations)).toBe("Đà Nẵng");
    });
  ...
  describe("formatCurrency", () => {
    ...
    it("định dạng ngân sách VND", () => {
      expect(formatCurrency(3000000)).toBe(formatOptionalVnd(3000000));
    });
  });
  ...
  describe("readPlannerApiResponse", () => {
    ...
    it("trả lỗi chuẩn hóa và log khi response không phải JSON", async () => {
      ...
      await expect(readPlannerApiResponse(response)).resolves.toEqual({
        success: false,
        message: "Server trả về dữ liệu không hợp lệ (502).",
      });
      expect(consoleErrorSpy).toHaveBeenCalledWith("[TRAVEL CHAT NON JSON RESPONSE]", {
        status: 502,
        url: "",
        body: "x".repeat(500),
      });
    ...
    it("xem response không có content-type là non-JSON", async () => {
      ...
      await expect(readPlannerApiResponse(response)).resolves.toEqual({
        success: false,
        message: "Server trả về dữ liệu không hợp lệ (503).",
      });
    });
    ...
    it("truyền lỗi parse lên trên khi content-type JSON nhưng body hỏng", async () => {
      ...
      await expect(readPlannerApiResponse(response)).rejects.toBeInstanceOf(SyntaxError);
    });
  });
});
