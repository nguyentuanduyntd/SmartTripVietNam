...
import type { HotelSearchItem, HotelSearchResult } from "@/src/components/planner/ai/chat/ai-travel-chat.types";
...
function formatRating(rating?: number) {
  ...
  const value = rating.toFixed(rating % 1 === 0 ? 0 : 1);
  return rating > 5 ? `${value}/10` : `${value}★`;
}
...
function getOverBudgetAmount(hotel: HotelSearchItem, maxPricePerNight?: number) {
  if (!maxPricePerNight || hotel.currency !== "VND" || hotel.pricePerNight === undefined) {
    return null;
  }

  const difference = hotel.pricePerNight - maxPricePerNight;
...
export function HotelChatCard({ result }: { result: HotelSearchResult }) {
  ...
  const isBudgetEmpty = result.items.length === 0;

  const displayedItems = isBudgetEmpty && showNearBudget ? nearBudgetItems : result.items;
  ...
  return (
    <div className="w-full overflow-hidden rounded-[26px] border border-[#dfd6c9] bg-white shadow-[0_16px_44px_rgba(23,58,59,0.08)]">
      <div className="border-b border-[#ebe2d6] bg-[#f8f3ea] px-5 py-4">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h3 className="mt-1 font-display text-xl font-semibold text-[#173a3b]">Chỗ ở tại {result.locationName}</h3>
          </div>
        ...
        <div className="mt-3 flex flex-wrap gap-2 text-xs font-bold text-[#667672]">
          ...
          {result.maxPricePerNight ? (
            <span className="inline-flex items-center gap-1.5 rounded-full bg-white px-3 py-1.5">
              <WalletCards size={13} />≤ {formatMoney(result.maxPricePerNight)}
              /đêm
            </span>
          ) : null}
        </div>
        ...
        {result.aiRecommendation ? (
          <div className="mt-3 flex items-start gap-2 rounded-2xl border border-[#d8e7df] bg-white/80 px-3.5 py-3 text-xs leading-5 text-[#536a64]">
            <Sparkles size={14} className="mt-0.5 shrink-0 text-[#d85b48]" />
            <div>
              <span className="font-extrabold text-[#173a3b]">
                {result.aiRecommendation.generatedBy === "gemini" ? "AI gợi ý: " : "SmartTrip gợi ý: "}
              </span>
      ...
      <div className="space-y-3 p-4 sm:p-5">
        {isBudgetEmpty ? (
          <div className="rounded-2xl border border-[#eadfce] bg-[#fffaf3] px-4 py-4">
            <p className="font-extrabold text-[#173a3b]">Chưa có chỗ ở phù hợp ngân sách</p>

            <p className="mt-2 text-sm leading-6 text-[#687873]">
              {result.message ?? "LiteAPI chưa trả về khách sạn phù hợp với bộ lọc hiện tại."}
            </p>

            {nearBudgetItems.length > 0 ? (
              <button
                type="button"
                onClick={() => setShowNearBudget((current) => !current)}
              ...
              >
                <WalletCards size={14} />
                {showNearBudget ? "Ẩn lựa chọn gần ngân sách" : `Xem ${nearBudgetItems.length} lựa chọn gần ngân sách`}
              </button>
            ) : null}
          </div>
        ) : null}

        {isBudgetEmpty && showNearBudget ? (
          <div className="flex items-center justify-between gap-3 px-1 pt-1">
            <p className="text-xs font-extrabold uppercase tracking-[0.12em] text-[#75817e]">Giá gần ngân sách nhất</p>

            <span className="text-[11px] font-semibold text-[#9a6d5d]">Các giá dưới đây vượt mức bạn đặt</span>
          </div>
        ) : null}
        ...
        {displayedItems.map((hotel) => {
          ...
          const googleMapsUrl = hotel.googleMapsUrl ?? buildGoogleMapsUrl(hotel, result.locationName);

          const overBudgetAmount = getOverBudgetAmount(hotel, result.maxPricePerNight);

          return (
            <article
              key={`${hotel.hotelId}-${hotel.offerId ?? "offer"}`}
              className={`overflow-hidden rounded-2xl border bg-white ${
                hotel.aiRecommended ? "border-[#cfded7] ring-1 ring-[#e5eee9]" : "border-[#e8e0d5]"
              }`}
            >
              <div className="flex flex-col sm:flex-row">
                {hotel.imageUrl ? (
                  <div className="h-40 w-full shrink-0 overflow-hidden bg-[#edf1ef] sm:h-auto sm:w-40">
                    {}
                    <img src={hotel.imageUrl} alt={hotel.name} className="h-full w-full object-cover" loading="lazy" />
                  </div>
                ) : null}
                <div className="min-w-0 flex-1 p-4">
                    ...
                    <div className="mb-2 flex flex-wrap items-center gap-2">
                      ...
                      {overBudgetAmount ? (
                        <span className="rounded-full bg-[#f8eee8] px-2.5 py-1 text-[10px] font-bold text-[#9a654f]">
                          Vượt {formatMoney(overBudgetAmount, "VND")}
                          /đêm
                        </span>
                      ) : null}
                    </div>
                  ...
                  {hotel.aiRecommended ? (
                    <div className="mb-2 flex flex-wrap items-center gap-2">
                      <span className="inline-flex items-center gap-1 rounded-full bg-[#edf6f3] px-2.5 py-1 text-[10px] font-extrabold uppercase tracking-wide text-[#34706b]">
                        <Sparkles size={11} />
                        {result.aiRecommendation?.generatedBy === "gemini" ? "AI đề xuất" : "SmartTrip đề xuất"} #
                        {hotel.recommendationRank ?? 1}
                      </span>
                  ...
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="shrink-0 text-right">
                      ...
                      <p className="mt-0.5 text-[11px] font-semibold text-[#87908e]">/ đêm</p>
                    </div>
                  </div>
                  ...
                  {hotel.roomDescription ? (
                    <p className="mt-3 line-clamp-2 text-xs leading-5 text-[#6f7d79]">{hotel.roomDescription}</p>
                  ) : null}
                  <div className="mt-3 flex flex-wrap items-center gap-2 text-[11px] font-bold">
                    {hotel.boardName ? (
                      <span className="rounded-full bg-[#edf6f3] px-2.5 py-1 text-[#46736d]">{hotel.boardName}</span>
                    ) : null}
                    ...
                    {hotel.refundable === false ? (
                      <span className="rounded-full bg-[#f6eee9] px-2.5 py-1 text-[#8a6658]">Không hoàn hủy</span>
                    ) : null}
                    ...
                    {hotel.totalPrice ? (
                      <span className="rounded-full bg-[#f5efe6] px-2.5 py-1 text-[#6f766f]">
                        Tổng {formatMoney(hotel.totalPrice, hotel.currency)}
                      </span>
                    ) : null}
                  </div>
      ...
      <div className="border-t border-[#ebe2d7] bg-[#fcf8f1] px-5 py-3 text-[11px] leading-5 text-[#7a8581]">
        Nguồn: {result.sourceLabel}. Giá và tình trạng phòng có thể thay đổi theo thời điểm tìm kiếm. SmartTrip chỉ hiển
        thị giá provider trả về; AI chỉ xếp hạng và giải thích lựa chọn, không tự bịa giá phòng.
      </div>
    </div>
  );
}
