...
function notifyHeaderProfileUpdated(detail: { fullName?: string; avatarUrl?: string | null; role?: "user" | "admin" }) {
  window.dispatchEvent(new CustomEvent("smarttrip:profile-updated", { detail }));
}

export function ProfileSettings({ initialFullName, email, initialAvatarUrl, role }: ProfileSettingsProps) {
  ...
  const hasNameChanged = normalizedName !== savedFullName;
  const displayedAvatar = avatarPreview ?? avatarUrl;
  const initials = getInitials(normalizedName || email);
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const payload = (await response.json()) as ApiPayload<UpdatedProfile>;

      if (!response.ok || !payload.success || !payload.data) {
        throw new Error(payload.errors?.fullName?.[0] ?? payload.message ?? "Chưa thể cập nhật thông tin.");
      }

      const nextFullName = payload.data.fullName ?? normalizedName;

      setFullName(nextFullName);
      setSavedFullName(nextFullName);
      setMessage({
        type: "success",
        text: payload.message ?? "Cập nhật thông tin thành công.",
      });
      ...
      setMessage({
        type: "error",
        text: error instanceof Error ? error.message : "Chưa thể cập nhật thông tin.",
      });
    } finally {
      setIsSaving(false);
    }
  ...
  async function handleAvatarChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const payload = (await response.json()) as ApiPayload<UpdatedAvatar>;

      if (!response.ok || !payload.success || !payload.data) {
        throw new Error(payload.message ?? "Chưa thể cập nhật ảnh đại diện.");
      }

      setAvatarUrl(payload.data.avatarUrl);
      setMessage({
        type: "success",
        text: payload.message ?? "Cập nhật ảnh đại diện thành công.",
      });
      ...
      setMessage({
        type: "error",
        text: error instanceof Error ? error.message : "Chưa thể cập nhật ảnh đại diện.",
      });
    } finally {
  ...
  async function handleRemoveAvatar() {
    ...
    try {
      ...
      if (!response.ok || !payload.success) {
        throw new Error(payload.message ?? "Chưa thể xóa ảnh đại diện.");
      }

      setAvatarUrl(null);
      setMessage({
        type: "success",
        text: payload.message ?? "Đã xóa ảnh đại diện.",
      });
      ...
      setMessage({
        type: "error",
        text: error instanceof Error ? error.message : "Chưa thể xóa ảnh đại diện.",
      });
    } finally {
      setIsAvatarBusy(false);
    }
    ...
    <div className="grid gap-6 lg:grid-cols-[340px_minmax(0,1fr)]">
        ...
        <div className="text-center">
          <div className="relative mx-auto h-36 w-36">
            ...
            <span className="absolute bottom-1 right-1 grid h-11 w-11 place-items-center rounded-full border-4 border-[#fffaf3] bg-[#f3bd59] text-[#173a3b]">
              {isAvatarBusy ? <Loader2 size={18} className="animate-spin" /> : <Camera size={18} />}
            </span>
          </div>

          <h2 className="mt-6 font-display text-3xl font-semibold text-[#173a3b]">{normalizedName}</h2>
          <p className="mt-1 break-all text-sm text-[#71807c]">{email}</p>
      ...
      >
        <div className="flex items-start gap-4 border-b border-[#e9dfd2] pb-6">
          ...
          <div>
            <h2 className="font-display text-3xl font-semibold text-[#173a3b]">Thông tin cơ bản</h2>
        ...
        <div className="mt-7 grid gap-6">
          <label className="block">
            <span className="text-sm font-bold text-[#294748]">Họ và tên</span>
          ...
          <label className="block">
            <span className="text-sm font-bold text-[#294748]">Email</span>
            <div className="relative mt-2">
              <Mail size={18} className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-[#7b8a86]" />
              <input
                value={email}
                readOnly
                aria-readonly="true"
