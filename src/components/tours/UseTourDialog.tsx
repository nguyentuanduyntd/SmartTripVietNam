...
import { BedDouble, CalendarDays, LoaderCircle, Route, UserRound, UsersRound, X } from "lucide-react";
import { useRouter } from "next/navigation";
import { useEffect, useId, useState } from "react";
import { createPortal } from "react-dom";
import { useForm } from "react-hook-form";

import { itinerarySetupFormSchema, type ItinerarySetupForm } from "@/src/db/schema/itinerary.schema";
...
function getDefaultValues(tourName: string): ItinerarySetupForm {
  return {
    title: tourName,
...
export function UseTourDialog({ sourceTourId, tourName }: UseTourDialogProps) {
  ...
  const {
    ...
    resolver: zodResolver(itinerarySetupFormSchema),
  ...
  function applyServerFieldErrors(payload: unknown) {
  ...
  async function submitForm(values: ItinerarySetupForm) {
    clearErrors();

    try {
      const response = await fetch("/api/itineraries/from-tour", {
        method: "POST",
        headers: {
      ...
      if (response.status === 401) {
        const returnPath = [window.location.pathname, window.location.search, window.location.hash].join("");
      ...
      if (!response.ok) {
        ...
        setError("root.server", {
          type: "server",
          message: getApiMessage(payload, "Không thể tạo hành trình. Vui lòng thử lại."),
        });

        return;
      }

      const redirectTo = getRedirectPath(payload);

      if (!redirectTo) {
        setError("root.server", {
          type: "server",
          message: "Hành trình đã được tạo nhưng máy chủ không trả về đường dẫn hợp lệ.",
        });
      ...
      console.error("Không thể tạo hành trình:", error);
      ...
      ? createPortal(
          ...
          >
            ...
            >
              <header className="flex shrink-0 items-start justify-between gap-5 border-b border-[#e2d8c9] bg-[#fffaf1] px-5 py-5 sm:px-7 sm:py-6">
                <div className="min-w-0">
                  <p className="text-xs font-extrabold uppercase tracking-[0.18em] text-[#d85b48]">
                    Tạo bản hành trình riêng
                  </p>

                  <h2 id={titleId} className="mt-2 font-display text-2xl font-semibold text-[#173a3b] sm:text-3xl">
                    Thiết lập chuyến đi
                  </h2>

                  <p id={descriptionId} className="mt-2 max-w-xl text-sm leading-6 text-[#687572]">
                    Hệ thống sẽ sao chép tour mẫu thành một bản riêng trong tài khoản. Các chỉnh sửa sau đó không ảnh
                    hưởng tới tour gốc.
                  </p>
                </div>
              ...
              <form onSubmit={handleSubmit(submitForm)} className="flex min-h-0 flex-1 flex-col" noValidate>
                <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-5 py-5 sm:px-7 sm:py-6">
                  <div>
                    <label htmlFor="itinerary-title" className="mb-2 block text-sm font-bold text-[#294748]">
                      Tên hành trình
                    </label>
                    ...
                    {errors.title ? <p className="mt-2 text-sm text-[#b54c3d]">{errors.title.message}</p> : null}
                  </div>

                  <div className="mt-5">
                    <label htmlFor="itinerary-start-date" className="mb-2 block text-sm font-bold text-[#294748]">
                      Ngày khởi hành
                    </label>
                    ...
                    {errors.startDate ? (
                      <p className="mt-2 text-sm text-[#b54c3d]">{errors.startDate.message}</p>
                    ) : null}
                  </div>

                  <div className="mt-5 grid gap-4 sm:grid-cols-3">
                    <div>
                      <label htmlFor="adult-count" className="mb-2 block text-sm font-bold text-[#294748]">
                        Người lớn
                      </label>
                      ...
                      {errors.adultCount ? (
                        <p className="mt-2 text-sm text-[#b54c3d]">{errors.adultCount.message}</p>
                      ) : null}
                    </div>

                    <div>
                      <label htmlFor="child-count" className="mb-2 block text-sm font-bold text-[#294748]">
                        Trẻ em
                      </label>
                      ...
                      {errors.childCount ? (
                        <p className="mt-2 text-sm text-[#b54c3d]">{errors.childCount.message}</p>
                      ) : null}
                    </div>

                    <div>
                      <label htmlFor="room-count" className="mb-2 block text-sm font-bold text-[#294748]">
                        Số phòng
                      </label>
                      ...
                      {errors.roomCount ? (
                        <p className="mt-2 text-sm text-[#b54c3d]">{errors.roomCount.message}</p>
                      ) : null}
                    </div>
                  </div>
                  ...
                  <div className="mt-7 rounded-2xl border border-[#d9e5df] bg-[#edf5f1] px-4 py-3 text-sm leading-6 text-[#54706c]">
                    Chi phí tour hiện tại sẽ được sao chép thành khoản tham khảo theo đầu người. Bạn có thể chia nhỏ
                    thành vé, ăn uống, di chuyển và khách sạn trong trang chỉnh sửa.
                  </div>
                </div>
