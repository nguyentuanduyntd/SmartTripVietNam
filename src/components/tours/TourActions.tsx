...
const SAVED_TOURS_EVENT = "smarttrip:saved-tours-change";
...
function readSavedTourIds(): string[] {
  ...
  try {
    const rawValue = window.localStorage.getItem(SAVED_TOURS_KEY);
...
function subscribeSavedTours(callback: () => void) {
  const handleChange = () => {
    callback();
  };
  ...
  window.addEventListener(SAVED_TOURS_EVENT, handleChange);

  return () => {
    window.removeEventListener("storage", handleChange);

    window.removeEventListener(SAVED_TOURS_EVENT, handleChange);
  };
}

function notifySavedToursChanged() {
  window.dispatchEvent(new Event(SAVED_TOURS_EVENT));
}

export function TourActions({ tourId, name }: TourActionsProps) {
  ...
  function toggleSaved() {
    ...
    const nextIsSaved = !currentIds.includes(tourId);

    const nextIds = nextIsSaved ? [...currentIds, tourId] : currentIds.filter((id) => id !== tourId);

    try {
      window.localStorage.setItem(SAVED_TOURS_KEY, JSON.stringify(nextIds));

      notifySavedToursChanged();
    } catch (error) {
      console.error("Không thể lưu hành trình:", error);
    }
  }
  ...
  async function shareTour() {
    ...
    try {
      ...
      if (!navigator.clipboard) {
        throw new Error("Trình duyệt không hỗ trợ sao chép liên kết.");
      }
      ...
      console.error("Không thể chia sẻ hành trình:", error);
  ...
  return (
    <div className="grid gap-3">
      <UseTourDialog sourceTourId={tourId} tourName={name} />
      ...
      <p className="px-2 text-center text-[11px] leading-5 text-[#8a8575]">
        Bản lưu nhanh được giữ trên thiết bị. Hành trình tạo từ tour sẽ được lưu trong tài khoản.
      </p>
    </div>
  );
}
