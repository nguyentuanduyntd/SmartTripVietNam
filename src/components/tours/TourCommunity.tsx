...
export function TourCommunity({ tourId, tourName }: TourCommunityProps) {
  ...
  const loadCommunity = useCallback(async () => {
    try {
      const response = await fetch(`/api/tours/${tourId}/community`, {
        method: "GET",
        cache: "no-store",
      });

      const payload = (await response.json()) as ApiPayload<CommunityData>;

      if (!response.ok || !payload.success || !payload.data) {
        throw new Error(payload.message ?? "Không thể tải nhận xét");
      }

      setCommunity(payload.data);

      setError(null);
    } catch (loadError) {
      console.error(loadError);

      setError(loadError instanceof Error ? loadError.message : "Không thể tải lượt thích và nhận xét");
    } finally {
      setIsLoading(false);
    }
  }, [tourId]);
  ...
  async function handleLike() {
    ...
    const nextLiked = !community.likedByMe;

    setCommunity({
      ...community,
      likedByMe: nextLiked,
      likeCount: Math.max(0, community.likeCount + (nextLiked ? 1 : -1)),
    });
    ...
    try {
      ...
      if (!response.ok || !payload.success) {
        throw new Error(payload.message ?? "Không thể cập nhật lượt thích");
      }
      ...
      setError(likeError instanceof Error ? likeError.message : "Không thể cập nhật lượt thích");
    } finally {
      setIsLiking(false);
    }
  }
  ...
  async function handleSubmitComment(event: FormEvent<HTMLFormElement>) {
    ...
    if (!normalized) {
      setError("Bạn hãy nhập nội dung nhận xét.");
    ...
    try {
      ...
      if (!response.ok || !payload.success) {
        throw new Error(payload.message ?? "Không thể gửi nhận xét");
      }
      ...
      setError(submitError instanceof Error ? submitError.message : "Không thể gửi nhận xét");
    } finally {
      setIsSubmittingComment(false);
    }
  }

  function startEdit(item: CommunityComment) {
  ...
  async function saveEdit(commentId: string) {
    ...
    try {
      const response = await fetch(`/api/tour-comments/${commentId}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (!response.ok || !payload.success) {
        throw new Error(payload.message ?? "Không thể cập nhật nhận xét");
      }
      ...
      setError(editError instanceof Error ? editError.message : "Không thể cập nhật nhận xét");
    } finally {
  ...
  async function handleDeleteComment(commentId: string) {
    const confirmed = window.confirm("Bạn có chắc muốn xóa nhận xét này?");
    ...
    try {
      ...
      if (!response.ok || !payload.success) {
        throw new Error(payload.message ?? "Không thể xóa nhận xét");
      }

      await loadCommunity();
    } catch (deleteError) {
      setError(deleteError instanceof Error ? deleteError.message : "Không thể xóa nhận xét");
    } finally {
      ...
      <section className="border-t border-[#e4dacb] bg-[#fffaf1] px-5 py-14 sm:px-8 lg:px-12">
        <div className="mx-auto max-w-[1100px]">
          <p className="rounded-2xl border border-[#efd1ca] bg-[#fff2ef] p-4 text-sm text-[#a84e40]">
            {error ?? "Không thể tải dữ liệu cộng đồng."}
          </p>
        </div>
      </section>
    );
    ...
    >
      <div className="mx-auto max-w-[1100px]">
        ...
        <div className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-xs font-extrabold uppercase tracking-[0.2em] text-[#d85b48]">Cộng đồng</p>

            <h2 className="mt-2 font-display text-3xl font-semibold tracking-[-0.02em] text-[#173a3b] sm:text-4xl">
              Mọi người nói gì về chuyến đi?
            </h2>

            <p className="mt-3 max-w-2xl text-sm leading-7 text-[#6b7976] sm:text-base">
              Chia sẻ cảm nhận của bạn về <strong className="font-semibold text-[#173a3b]">{tourName}</strong>.
            </p>
          </div>
          ...
          >
            {isLiking ? (
              ...
              <Heart size={20} fill={community.likedByMe ? "currentColor" : "none"} />
            )}

            {community.likedByMe ? "Đã thích" : "Thích tour này"}
          </button>
        </div>
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[0.78fr_1.22fr]">
          ...
          <div>
            <div className="rounded-[28px] border border-[#ded3c3] bg-[#f7f0e4] p-5 sm:p-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-display text-xl font-semibold text-[#173a3b]">Viết nhận xét</h3>

                  <p className="mt-0.5 text-xs text-[#78837f]">Chia sẻ trải nghiệm của bạn</p>
                </div>
              </div>
              ...
              {community.currentUserId ? (
                <form onSubmit={handleSubmitComment} className="mt-5">
                  ...
                  <div className="mt-2 flex items-center justify-between gap-3">
                    ...
                    >
                      {isSubmittingComment ? <Loader2 size={17} className="animate-spin" /> : <Send size={17} />}
                      Gửi nhận xét
                    </button>
                  </div>
                </form>
                ...
                <div className="mt-5 rounded-2xl border border-dashed border-[#cfc2b1] bg-white/70 p-5">
                  <p className="text-sm leading-6 text-[#6d7a77]">
                    Bạn cần đăng nhập để thả tim và viết nhận xét về tour.
                  </p>
              ...
              {error ? (
                <p className="mt-4 rounded-xl bg-[#fff0ed] px-4 py-3 text-sm leading-6 text-[#b24e40]">{error}</p>
              ) : null}
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between gap-4">
              <h3 className="font-display text-2xl font-semibold text-[#173a3b]">Nhận xét gần đây</h3>

              <span className="text-sm text-[#798682]">{community.commentCount} nhận xét</span>
            </div>
            ...
            {community.comments.length === 0 ? (
              <div className="mt-5 rounded-[28px] border border-dashed border-[#d4c8b8] bg-[#faf5ec] px-6 py-12 text-center">
                <MessageCircle size={32} className="mx-auto text-[#8ca5a0]" />

                <h4 className="mt-4 font-display text-xl font-semibold text-[#173a3b]">Chưa có nhận xét</h4>

                <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-[#75817e]">
                  Hãy là người đầu tiên chia sẻ cảm nhận về chuyến tour này.
                </p>
              </div>
            ) : (
              <div className="mt-5 space-y-4">
                {community.comments.map((item) => {
                  const isOwner = community.currentUserId === item.userId;
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        {item.user.avatarUrl ? (
                          <img
                            ...
                            alt={item.user.fullName ?? "Người dùng"}
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex flex-wrap items-start justify-between gap-3">
                            <div>
                              <p className="font-bold text-[#173a3b]">{item.user.fullName ?? "Người dùng"}</p>
