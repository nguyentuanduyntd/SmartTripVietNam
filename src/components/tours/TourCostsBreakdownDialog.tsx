...
import { ChevronRight, ReceiptText, WalletCards, X } from "lucide-react";
...
export function TourCostBreakdownDialog({ costs, durationNights, estimatedPrice }: TourCostBreakdownDialogProps) {
  ...
  const calculationContext = useMemo(
    () => ({
      ...
      defaultNightCount: durationNights,
    }),
    [durationNights],
  );

  const calculatedTotal = useMemo(
    () => (costs.length > 0 ? calculateCostsTotal(costs, calculationContext) : Number(estimatedPrice) || 0),
    [calculationContext, costs, estimatedPrice],
  );

  const costsByCategory = useMemo(() => {
    const result = new Map<CostCategory, TourCost[]>();
  ...
  const categoryTotals = useMemo(() => {
    ...
    for (const category of CATEGORY_ORDER) {
      ...
      result[category] = categoryCosts.reduce(
        (total, cost) => total + calculateCostAmount(cost, calculationContext),
        0,
      );
    }

    return result;
  }, [calculationContext, costsByCategory]);

  const visibleCategories = CATEGORY_ORDER.filter((category) => (costsByCategory.get(category) ?? []).length > 0);
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center justify-between gap-2">
              <p className="text-xs font-semibold text-[#8a8575]">Chi phí dự kiến</p>
            ...
            <p className="mt-1 truncate font-bold text-[#173a3b]">{formatCurrency(calculatedTotal)}</p>

            <p className="mt-1 text-[10px] font-semibold text-[#9b9483]">Xem chi tiết</p>
          </div>
        </div>
      </button>
            ...
            <header className="flex items-start justify-between gap-4 border-b border-[#e2d8c9] px-5 py-5 sm:px-6">
              <div>
                <div className="flex items-center gap-2 text-[#d85b48]">
                  ...
                  <p className="text-xs font-extrabold uppercase tracking-[0.16em]">Chi phí tour</p>
                </div>

                <h2 id="tour-cost-breakdown-title" className="mt-2 font-display text-2xl font-semibold text-[#173a3b]">
                  Chi tiết dự toán
                </h2>

                <p className="mt-1 text-sm leading-6 text-[#71807d]">
                  Mức tham khảo được tính cho 1 người lớn và 1 phòng.
                </p>
              </div>
            ...
            <div className="max-h-[calc(90vh-180px)] overflow-y-auto px-5 py-5 sm:px-6">
              {costs.length === 0 ? (
                <div className="rounded-2xl border border-dashed border-[#d7cabb] bg-white px-5 py-9 text-center">
                  <WalletCards size={30} className="mx-auto text-[#8b9995]" strokeWidth={1.5} />

                  <p className="mt-3 font-bold text-[#173a3b]">Tour chưa có breakdown chi phí</p>

                  <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-[#71807d]">
                    Giá hiện tại là mức tham khảo cũ. Chi tiết từng khoản đang được cập nhật.
                  </p>
                        ...
                        <div className="overflow-hidden rounded-2xl border border-[#e1d7ca] bg-white">
                          {categoryCosts.map((cost, index) => {
                            ...
                            return (
                              <div
                                key={cost.id}
                                className={`px-4 py-4 ${index > 0 ? "border-t border-[#eee5d9]" : ""}`}
                              >
            ...
            <footer className="border-t border-[#e2d8c9] bg-white/60 px-5 py-4 sm:px-6">
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="mt-0.5 text-xs text-[#7d8986]">Mức tham khảo cho một người</p>
                </div>

                <p className="font-display text-2xl font-semibold text-[#d85b48]">{formatCurrency(calculatedTotal)}</p>
              </div>
            </footer>
          </section>
        </div>
