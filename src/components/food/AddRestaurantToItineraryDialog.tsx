...
function getSuggestedUnitPrice(restaurant: RestaurantForItinerary) {
...
function getDayDateLabel(startDate: string | null, dayNumber: number) {
  if (!startDate) {
    return null;
  }
...
export function AddRestaurantToItineraryDialog({
  ...
  const [itineraries, setItineraries] = useState<FoodTargetItinerary[]>([]);
  ...
  const [successResult, setSuccessResult] = useState<AddToItineraryResult | null>(null);
  ...
  const selectedDay = useMemo(
    () => selectedItinerary?.days.find((day) => day.id === selectedDayId) ?? null,
  ...
  const estimatedAddedAmount =
    Number.isFinite(numericUnitPrice) && numericUnitPrice >= 0
      ? numericUnitPrice * (selectedItinerary?.travelerCount ?? 0)
      : 0;
  ...
  useEffect(() => {
    ...
    async function loadTargets() {
      ...
      try {
        const response = await fetch("/api/itineraries/food-targets", {
          cache: "no-store",
          signal: controller.signal,
        });

        const payload = (await response.json()) as ApiPayload<FoodTargetsResult>;
        ...
        const firstItinerary = items[0];

        setSelectedItineraryId(firstItinerary?.id ?? "");

        setSelectedDayId(firstItinerary?.days[0]?.id ?? "");
      } catch (loadError) {
        if (loadError instanceof DOMException && loadError.name === "AbortError") {
          return;
        }

        console.error("[FOOD ITINERARY TARGETS UI ERROR]", loadError);

        setItineraries([]);
        setSelectedItineraryId("");
        setSelectedDayId("");

        setError(loadError instanceof Error ? loadError.message : "Chưa thể tải danh sách lịch trình.");
      } finally {
    ...
    const timeoutId = window.setTimeout(() => {
      setMealType("lunch");
      setStartTime("12:00");
      setUnitPrice(String(getSuggestedUnitPrice(restaurant)));
      setError(null);
  ...
  function handleItineraryChange(itineraryId: string) {
  ...
  function handleMealTypeChange(value: MealType) {
    setMealType(value);

    const option = MEAL_OPTIONS.find((item) => item.value === value);

    if (option) {
      setStartTime(option.defaultTime);
    }
  }
  ...
  async function handleSubmit() {
    ...
    if (!selectedItinerary || !selectedDay) {
      setError("Bạn cần chọn lịch trình và ngày muốn thêm món.");
      return;
    }

    if (!Number.isFinite(numericUnitPrice) || numericUnitPrice < 0) {
      setError("Giá tham khảo không hợp lệ.");
      return;
    }

    if (!startTime) {
      setError("Bạn cần chọn khung giờ.");
      return;
    }
    ...
    try {
      const response = await fetch(`/api/restaurants/${encodeURIComponent(restaurantId)}/add-to-itinerary`, {
        method: "POST",
        headers: {
      ...
      const payload = (await response.json()) as ApiPayload<AddToItineraryResult>;

      if (!response.ok || !payload.success || !payload.data) {
        throw new Error(payload.message ?? "Chưa thể thêm món vào lịch trình.");
      }

      setSuccessResult(payload.data);
    } catch (submitError) {
      console.error("[ADD RESTAURANT TO ITINERARY UI ERROR]", submitError);

      setError(submitError instanceof Error ? submitError.message : "Chưa thể thêm món vào lịch trình.");
    } finally {
          ...
          <div className="min-w-0">
            ...
            {restaurant.address ? (
              <p className="mt-2 flex items-start gap-1.5 text-xs leading-5 text-[#74817e]">
                <MapPin size={13} className="mt-0.5 shrink-0 text-[#e05e4c]" />
                {restaurant.address}
              </p>
            ) : null}
          </div>
        ...
        {successResult ? (
          <div className="p-5 sm:p-7">
            <div className="rounded-[26px] border border-[#cfe2d8] bg-[#f1f8f4] p-5 sm:p-6">
              ...
              <h3 className="mt-4 font-display text-2xl font-semibold text-[#173a3b]">Đã thêm vào lịch trình</h3>

              <p className="mt-2 text-sm leading-6 text-[#61736e]">
                {restaurant.name} đã được thêm vào{" "}
                <strong className="text-[#315f5f]">Ngày {successResult.day.dayNumber}</strong> của{" "}
                <strong className="text-[#315f5f]">{successResult.itinerary.title}</strong>.
              </p>
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-2">
                <div className="rounded-2xl border border-[#d9e7df] bg-white px-4 py-4">
                  ...
                  <p className="mt-1 text-xl font-extrabold text-[#d85b48]">
                    {formatMoney(successResult.cost.addedAmount)}
                  </p>
                </div>
                ...
                <div className="rounded-2xl border border-[#d9e7df] bg-white px-4 py-4">
                  ...
                  <p className="mt-1 text-xl font-extrabold text-[#173a3b]">
                    {formatMoney(successResult.costSummary.total)}
                  </p>
                </div>
              </div>
              ...
              <div className="mt-5 flex flex-col gap-2 sm:flex-row sm:justify-end">
                ...
                <button
                  type="button"
                  onClick={() => router.push(successResult.redirectTo)}
          ...
          <div className="p-5 sm:p-7">
            {isLoadingTargets ? (
              <div className="flex min-h-[260px] items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="mt-3 text-sm font-bold text-[#60736d]">Đang tải lịch trình của bạn...</p>
                </div>
              </div>
              ...
              <>
                <div className="grid gap-5">
                  <label className="block">
                    ...
                    <select
                      ...
                      onChange={(event) => handleItineraryChange(event.target.value)}
                  ...
                  {selectedItinerary ? (
                    <div className="flex flex-wrap items-center gap-2 rounded-2xl bg-[#f4f0e8] px-4 py-3 text-xs font-semibold text-[#687772]">
                      ...
                      {selectedItinerary.childCount > 0 ? <span>+ {selectedItinerary.childCount} trẻ em</span> : null}

                      <span className="font-extrabold text-[#315f5f]">= {selectedItinerary.travelerCount} khách</span>
                    </div>
                  ) : null}
                  ...
                  <label className="block">
                    ...
                    <select
                      ...
                      disabled={!selectedItinerary || selectedItinerary.days.length === 0}
                    ...
                    >
                      {selectedItinerary?.days.map((day) => {
                        const dateLabel = getDayDateLabel(selectedItinerary.startDate, day.dayNumber);
                ...
                <div className="mt-6 rounded-[24px] border border-[#d8e4df] bg-[#f1f8f5] p-4 sm:p-5">
                  <div className="flex items-start justify-between gap-5">
                    <div>
                      ...
                      <p className="mt-1 text-2xl font-extrabold text-[#d85b48]">{formatMoney(estimatedAddedAmount)}</p>
                    </div>

                    <div className="text-right text-xs leading-5 text-[#63756f]">
                      <p>
                        {formatMoney(Number.isFinite(numericUnitPrice) ? Math.max(numericUnitPrice, 0) : 0)} / người
                      </p>
                      <p className="font-extrabold text-[#315f5f]">× {selectedItinerary?.travelerCount ?? 0} khách</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-6 flex flex-col-reverse gap-2 border-t border-[#e7ddcf] pt-5 sm:flex-row sm:justify-end">
                  ...
                  <button
                    ...
                    disabled={isSubmitting || !selectedItinerary || !selectedDay}
            ...
            {error && itineraries.length === 0 && !isLoadingTargets ? (
              <div className="mt-5 rounded-2xl border border-[#edc8be] bg-[#fff4ef] px-4 py-3 text-sm leading-6 text-[#9a4c3d]">
                {error}
              </div>
            ) : null}
