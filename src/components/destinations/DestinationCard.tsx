...
export function DestinationCard({ destination, locationName }: DestinationCardProps) {
  const locale = useLocale();

  const name = localizedText(locale, {
    vi: destination.name,
      ...
      <div className="p-6 sm:p-7">
        ...
        <div className="mt-3 flex items-start justify-between gap-4">
          <div className="min-w-0">
            <h3 className="truncate font-display text-2xl font-semibold text-[#173a3b]">{name}</h3>

            {description ? <p className="mt-3 line-clamp-2 leading-7 text-[#667370]">{description}</p> : null}
