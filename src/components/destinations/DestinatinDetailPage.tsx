...
import { localizedNullableText, localizedText } from "@/src/i18n/localized-text";
...
export function DestinationDetailPage({ id }: DestinationDetailPageProps) {
      ...
      try {
        ...
        locationsApi
          ...
          .then((locations) => {
            ...
            setLocation(locations.find((item) => item.id === data.locationId) ?? null);
          })
          .catch((error) => {
            console.error("Failed to load destination location:", error);
          });
      } catch (error) {
        if (!active) {
          return;
    ...
    return (
      <main className="min-h-screen bg-[#fffaf1] text-[#173a3b]">
        ...
        <div className="mx-auto max-w-2xl px-5 pb-32 pt-40 text-center sm:px-8">
          <p className="font-display text-3xl font-semibold">{t("notFoundTitle")}</p>

          <p className="mt-4 text-[#60706d]">{t("notFoundDescription")}</p>
  ...
  if (loadError || !destination) {
    return (
      <main className="min-h-screen bg-[#fffaf1] text-[#173a3b]">
        ...
        <div className="mx-auto max-w-2xl px-5 pb-32 pt-40 text-center sm:px-8">
          <p className="font-display text-3xl font-semibold">{t("errorTitle")}</p>

          <p className="mt-4 text-[#60706d]">{t("loadError")}</p>
  ...
  const description = localizedNullableText(locale, {
    vi: destination.description,

    en: destination.descriptionEn,
  });

  const history = localizedNullableText(locale, {
    vi: destination.history,

    en: destination.historyEn,
  });
  ...
  const categories = destination.categories.map((category) => ({
    ...category,

    localizedName: localizedText(locale, {
      vi: category.name,
  ...
  const alternateNameRaw = locale === "en" ? destination.name : destination.nameEn;

  const alternateName =
    typeof alternateNameRaw === "string" &&
    alternateNameRaw.trim().length > 0 &&
    alternateNameRaw.trim() !== destinationName
      ...
      <section className="relative h-[62vh] min-h-[420px] w-full overflow-hidden">
        ...
        <div className="absolute inset-x-0 bottom-0 px-5 pb-10 sm:px-8 lg:px-12 lg:pb-14">
          <div className="mx-auto max-w-[1440px]">
            ...
            {alternateName ? <p className="mt-2 text-lg italic text-white/70">{alternateName}</p> : null}
            ...
            <div className="mt-5 flex flex-wrap items-center gap-4 text-white/85">
              ...
              {destination.address ? <span className="text-sm text-white/70">{destination.address}</span> : null}
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="px-5 py-16 sm:px-8 lg:px-12 lg:py-20">
        <div className="mx-auto grid max-w-[1440px] gap-12 lg:grid-cols-[1.6fr_1fr]">
          <div>
            {description ? (
              <div>
                <h2 className="font-display text-2xl font-semibold text-[#173a3b] sm:text-3xl">{t("introduction")}</h2>

                <p className="mt-4 whitespace-pre-line leading-8 text-[#4a5654]">{description}</p>
              </div>
            ) : null}
            ...
            {history ? (
              <div className="mt-12">
                <h2 className="flex items-center gap-2 font-display text-2xl font-semibold text-[#173a3b] sm:text-3xl">
                  <ScrollText size={22} className="text-[#e55c49]" />

                  {t("history")}
                </h2>

                <p className="mt-4 whitespace-pre-line leading-8 text-[#4a5654]">{history}</p>
              </div>
            ) : null}

            {!description && !history ? <p className="text-[#60706d]">{t("noDetails")}</p> : null}
          </div>

          <aside className="h-max rounded-[28px] border border-[#ded3c3] bg-[#f7f0e4] p-7">
            <h3 className="font-display text-xl font-semibold text-[#173a3b]">{t("quickInfo")}</h3>

            <dl className="mt-5 grid gap-4 text-sm">
              {locationName ? (
                <div>
                  <dt className="font-semibold text-[#8a8575]">{t("region")}</dt>

                  <dd className="mt-1 text-[#173a3b]">{locationName}</dd>
                </div>
              ) : null}

              {destination.address ? (
                <div>
                  <dt className="font-semibold text-[#8a8575]">{t("address")}</dt>

                  <dd className="mt-1 text-[#173a3b]">{destination.address}</dd>
                </div>
              ) : null}

              {categories.length > 0 ? (
                <div>
                  <dt className="font-semibold text-[#8a8575]">{t("categories")}</dt>
