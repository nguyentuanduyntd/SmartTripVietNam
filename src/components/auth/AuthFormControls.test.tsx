...
describe("AuthFormControls", () => {
  describe("AuthTextField", () => {
    it("liên kết label với input và nhận dữ liệu người dùng", async () => {
      ...
      expect(screen.getByText("Email đăng ký")).toBeInTheDocument();
    });

    it("đánh dấu input không hợp lệ", () => {
      render(<AuthTextField id="invalid-email" label="Email" icon={Mail} invalid />);
  ...
  describe("AuthPasswordField", () => {
    it("ẩn mật khẩu mặc định và cho phép hiện/ẩn", async () => {
      ...
      render(<AuthPasswordField id="password" label="Mật khẩu" />);
  ...
  it("hiển thị thông báo lỗi với role alert", () => {
    render(<AuthAlert>Email hoặc mật khẩu không đúng</AuthAlert>);
  ...
  it("đổi nội dung nút submit theo trạng thái loading", () => {
    const { rerender } = render(<AuthSubmitButton loadingLabel="Đang đăng nhập...">Đăng nhập</AuthSubmitButton>);
    ...
    rerender(
      <AuthSubmitButton loading disabled loadingLabel="Đang đăng nhập...">
        Đăng nhập
      </AuthSubmitButton>,
    );
    ...
    rerender(
      <AuthSubmitButton loadingLabel="Đang đăng nhập..." showArrow={false}>
        Đăng nhập
      </AuthSubmitButton>,
    );
  ...
  it("hiển thị divider và nút đăng nhập Google", () => {
    ...
    rerender(<GoogleAuthButton loading disabled />);
