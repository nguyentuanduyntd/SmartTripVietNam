...
import { ArrowLeft, Landmark, Sparkles, type LucideIcon } from "lucide-react";
    ...
    >
      <span
        className={joinClasses(
          ...
          isLargeInverse ? "h-12 w-12 shadow-[0_14px_35px_rgba(242,95,75,0.28)]" : "h-11 w-11",
        )}
      >
        <Landmark size={isLargeInverse ? 24 : 22} strokeWidth={isLargeInverse ? 1.8 : 2} />
      </span>

      <span className={joinClasses("font-display font-semibold", isLargeInverse ? "text-3xl" : "text-2xl")}>
        Rực Rỡ Miền Trung
      </span>
    </Link>
  );
...
type AuthBackgroundVariant = "split" | "centered" | "update" | "register-result" | "pattern-only";

function AuthBackground({ variant }: { variant: AuthBackgroundVariant }) {
  return (
    <>
...
export function AuthCenteredLayout({
  ...
  return (
    <main className="relative flex min-h-dvh items-center justify-center overflow-hidden bg-[#f7efe1] px-5 py-10 text-[#173a3b]">
      {backgroundVariant ? <AuthBackground variant={backgroundVariant} /> : null}
...
export function AuthCard({
  ...
  children: ReactNode;
  variant?: "split" | "form" | "form-plain" | "result" | "result-spacious";
}) {
  const variants = {
