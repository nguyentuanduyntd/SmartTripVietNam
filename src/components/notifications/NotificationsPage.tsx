...
import { notificationsApi, type NotificationListData, type UserNotification } from "@/src/lib/api-client/notifications";
import { formatVietnameseDateTime } from "@/src/lib/formatters";

function NotificationCard({ notification, onRead, markAsReadLabel }: { notification: UserNotification; onRead: (id: string) => void; markAsReadLabel: string }) {
        ...
        <div className="min-w-0 flex-1">
          <div className="flex flex-wrap items-start justify-between gap-2">
            <h2 className="font-bold text-[#173a3b]">
              {notification.title}
              {unread ? <span className="ml-2 inline-block h-2 w-2 rounded-full bg-[#f25f4b] align-middle" /> : null}
            </h2>
            <time className="text-xs text-[#7b8985]">{formatVietnameseDateTime(notification.createdAt)}</time>
          </div>
          <p className="mt-2 whitespace-pre-wrap text-sm leading-7 text-[#596b67]">{notification.message}</p>
          {unread ? (
            <button
              type="button"
...
export function NotificationsPage() {
  ...
  const loadNotifications = useCallback(async () => {
    ...
    try {
      ...
      setErrorMessage(error instanceof ApiRequestError ? error.message : t("loadError"));
    } finally {
      setLoading(false);
    }
  }, [page]);
  ...
  async function markOneRead(notificationId: string) {
    try {
      ...
      setErrorMessage(error instanceof ApiRequestError ? error.message : "Không thể cập nhật thông báo");
    }
  }
      ...
      <section className="rounded-[32px] border border-white/75 bg-[#fffaf1] p-6 shadow-[0_24px_80px_rgba(23,58,59,0.10)] sm:p-9">
        <div className="flex flex-col gap-4 border-b border-[#e0d6c7] pb-6 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="inline-flex items-center gap-2 text-xs font-extrabold uppercase tracking-[0.18em] text-[#d85b48]">
              <Bell size={15} /> {t("center")}
            </p>
            <h1 className="mt-3 font-display text-4xl font-semibold tracking-[-0.03em] text-[#173a3b]">
              {t("title")}
            </h1>
            <p className="mt-2 text-sm text-[#667572]">{t("unreadCount", { count: data?.unreadCount ?? 0 })}</p>
          </div>
          <button
            type="button"
        ...
        {loading ? (
          ...
          <div className="grid min-h-72 place-items-center text-center">
            <div>
              ...
              <p className="mt-3 font-bold text-[#173a3b]">{t("emptyTitle")}</p>
              <p className="mt-1 text-sm text-[#7b8985]">{t("emptyDescription")}</p>
            </div>
          </div>
        ) : (
        ...
        {data && data.pageCount > 1 ? (
          <div className="mt-6 flex items-center justify-between border-t border-[#e0d6c7] pt-5">
            <p className="text-xs text-[#7b8985]">
            {t("page", { current: data.page, total: data.pageCount })}
            </p>
            <div className="flex gap-2">
              <button
                ...
                className="rounded-full border border-[#cfc5b5] p-2 disabled:opacity-40"
              >
                <ChevronLeft size={17} />
              </button>
              <button
