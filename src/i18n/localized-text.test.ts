...
import { localizedNullableText, localizedText, localizedTextForLocale } from "@/src/i18n/localized-text";

describe("i18n localized text", () => {
  describe("isAppLocale", () => {
    it.each(["vi", "en"])("chấp nhận locale hợp lệ: %s", (locale) => {
      expect(isAppLocale(locale)).toBe(true);
    });

    it.each(["VI", "fr", "", null, undefined, 1])("từ chối locale không hợp lệ: %s", (locale) => {
  ...
  describe("localizedTextForLocale", () => {
    it("chọn nội dung bằng AppLocale đã xác định", () => {
      expect(localizedTextForLocale("en", "Ẩm thực", "Cuisine")).toBe("Cuisine");
    });

    it("hỗ trợ fallback tùy chỉnh", () => {
      expect(localizedTextForLocale("vi", null, null, "Chưa cập nhật")).toBe("Chưa cập nhật");
    });
  });
});
