...
const optionalLocationNameSchema = z
  ...
  .max(150, "Tên tiếng Anh không được vượt quá 150 ký tự")
  .nullable()
  .optional();
...
export const locationSlugSchema = z
  ...
  .regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/, "Slug chỉ được chứa chữ thường không dấu, số và dấu gạch ngang")
  .meta({ description: "Slug duy nhất của location", example: "da-nang" });
...
export const createLocationRequestSchema = z
  .object({
    name: locationNameSchema.meta({ example: "Đà Nẵng" }),
    nameEn: optionalLocationNameSchema.meta({ example: "Da Nang" }),
    slug: locationSlugSchema.optional(),
    description: optionalDescriptionSchema.meta({
      example: "Thành phố ven biển thuộc khu vực miền Trung Việt Nam.",
    }),
...
export const deleteLocationResponseSchema = z
  ...
  .meta({ id: "DeleteLocationResponse" });
...
export const locationDeleteConflictSchema = z
  ...
  .meta({
    ...
    description: "Location không thể bị xóa vì vẫn còn destination liên kết",
  });
