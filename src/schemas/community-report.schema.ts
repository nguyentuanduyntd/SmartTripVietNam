...
import { COMMUNITY_REPORT_REASONS, COMMUNITY_REPORT_STATUSES } from "@/src/constants/tour_community";

const communityReportTargetIdSchema = z.string().uuid("ID nội dung báo cáo không đúng định dạng UUID");
...
export const createCommunityReportSchema = z
  ...
  .superRefine((data, ctx) => {
    ...
    if (hasPostId === hasCommentId) {
      const message = "Báo cáo phải chọn đúng một đối tượng: bài viết hoặc bình luận";
...
export const communityReportIdParamSchema = z
  .object({ reportId: z.string().uuid("Report ID không đúng định dạng UUID") })
  .strict();

export type CommunityReportIdParam = z.infer<typeof communityReportIdParamSchema>;

export const communityReportStatusSchema = z.enum(COMMUNITY_REPORT_STATUSES);

export type CommunityReportStatusInput = z.infer<typeof communityReportStatusSchema>;
