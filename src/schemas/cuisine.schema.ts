...
const destinationIdsSchema = z
  .array(z.string().uuid("Destination ID không đúng định dạng UUID"))
  .optional()
  .meta({ description: "Danh sách ID địa danh gắn với món ăn" });

export const createCuisineRequestSchema = z
  .object({
...
export const updateCuisineRequestSchema = z
  .object({
...
export const cuisineIdParamsSchema = z
  .object({
    id: z
      .string()
  ...
  .strict()
  .meta({ id: "CuisineIdParams" });

export const cuisineListQuerySchema = z
  .object({
...
export const cuisineDestinationSummarySchema = z
  .object({
    id: z.string().uuid(),
    name: z.string(),
    slug: z.string(),
...
export const cuisineListResponseSchema = z
  .object({
    success: z.literal(true),
