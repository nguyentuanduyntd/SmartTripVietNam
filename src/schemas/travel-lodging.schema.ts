...
export const travelLodgingSearchSchema = z
  .object({
    ...
    requirements: z.array(z.string().trim().min(1).max(100)).max(12).default([]),
  })
  .superRefine((value, ctx) => {
    const checkIn = Date.parse(`${value.checkInDate}T00:00:00Z`);
    const checkOut = Date.parse(`${value.checkOutDate}T00:00:00Z`);
    ...
    if (value.childAges.length !== value.childCount) {
      ctx.addIssue({
        code: "custom",
        path: ["childAges"],
        message:
    ...
    if (value.roomCount > value.adultCount) {
      ctx.addIssue({
        code: "custom",
        path: ["roomCount"],
        message: "Mỗi phòng cần ít nhất 1 người lớn, nên số phòng không được lớn hơn số người lớn.",
      });
    }
  });

export type TravelLodgingSearchInput = z.infer<typeof travelLodgingSearchSchema>;
