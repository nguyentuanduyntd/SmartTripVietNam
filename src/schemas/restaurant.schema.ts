...
export const restaurantNearbyQuerySchema = z
  .object({
    ...
    cuisineId: z.string().uuid("Cuisine ID không đúng định dạng UUID").optional(),
    search: z.string().trim().min(1).max(120).optional(),
    maxPrice: z.coerce.number().int().positive().max(20_000_000).optional(),
    tags: tagsQuerySchema,
    openLate: booleanQuerySchema,
    familyFriendly: booleanQuerySchema,
    sort: z.enum(["best_match", "distance", "rating"]).default("best_match"),
    limit: z.coerce.number().int().min(1).max(30).default(12),
  })
  .strict();

export type RestaurantNearbyQuery = z.infer<typeof restaurantNearbyQuerySchema>;
