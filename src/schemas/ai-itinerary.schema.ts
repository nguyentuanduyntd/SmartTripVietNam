...
export const aiPlannerRequestSchema = z.object({
  ...
  startDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Ngày khởi hành không hợp lệ"),
...
const aiActivitySchema = z.object({
  ...
  endTime: timeSchema,
  transportMethod: z.enum(TRANSPORT_METHODS),
  estimatedTravelMinutes: z.number().int().min(0).max(360),
});

const aiCuisineSchema = z.object({
  cuisineId: z.string().uuid(),
  cuisineName: z.string().trim().min(1).max(200),
});
...
const aiMealSchema = z.object({
  ...
  cuisines: z.array(aiCuisineSchema).max(3),
});

const aiEstimatedCostSchema = z.object({
  title: z.string().trim().min(1).max(200),
  category: z.enum(COST_CATEGORIES),
  calculationUnit: z.enum(COST_CALCULATION_UNITS),
  travelerScope: z.enum(TRAVELER_SCOPES),
  unitPrice: z.number().int().min(0).max(100_000_000),
  quantity: z.number().positive().max(100).default(1),
  nightCount: z.number().int().min(1).max(30).nullable(),
  note: z.string().trim().max(500),
});
...
const aiDaySchema = z.object({
  ...
  title: z.string().trim().min(1).max(200),
  description: z.string().trim().min(1).max(1000),
  activities: z.array(aiActivitySchema).min(1).max(5),
...
export const aiItineraryPlanSchema = z.object({
  ...
  estimatedCosts: z.array(aiEstimatedCostSchema).min(1).max(30),
});
