...
import { formatVietnameseDate, formatVietnameseDateTime } from "@/src/lib/formatters";
...
export const metadata: Metadata = {
  ...
  description: "Xem và quản lý các hành trình du lịch đã tạo.",
};
...
export default async function MyItinerariesPage() {
  ...
  if (!user) {
    redirect("/auth/login?next=%2Fplanner");
  }

  const itineraries = await findUserItineraryList(user.id);
      ...
      <main className="min-h-screen bg-[#f4efe6] px-4 pb-16 pt-32 text-[#173a3b] sm:px-6 sm:pt-36 lg:px-8">
        <div className="mx-auto w-full max-w-7xl">
            ...
            <div className="relative overflow-hidden px-6 py-9 sm:px-9 lg:px-12 lg:py-11">
              ...
              <div className="relative flex flex-col gap-7 lg:flex-row lg:items-end lg:justify-between">
                <div className="max-w-3xl">
                  <p className="text-xs font-extrabold uppercase tracking-[0.2em] text-[#d85b48]">Không gian cá nhân</p>
                  ...
                  <p className="mt-4 max-w-2xl text-sm leading-7 text-[#667572] sm:text-base">
                    Tất cả hành trình bạn tạo từ tour mẫu sẽ tự động xuất hiện tại đây. Chọn một hành trình để xem chi
                    tiết và tiếp tục hoàn thiện chuyến đi.
                  </p>
                </div>
          ...
          <div className="mt-8 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="text-xs font-extrabold uppercase tracking-[0.18em] text-[#d85b48]">Danh sách đã lưu</p>

              <h2 className="mt-1 font-display text-3xl font-semibold">
                {itineraries.length === 0 ? "Chưa có lịch trình" : `${itineraries.length} lịch trình`}
              </h2>
            </div>

            {itineraries.length > 0 ? (
              <p className="text-sm text-[#71807d]">Sắp xếp theo lần cập nhật gần nhất</p>
            ) : null}
          </div>
          ...
          {itineraries.length === 0 ? (
            <section className="mt-6 rounded-[30px] border border-dashed border-[#cdbfae] bg-[#fffaf1] px-6 py-16 text-center shadow-[0_16px_50px_rgba(23,58,59,0.06)]">
              ...
              <h2 className="mt-5 font-display text-3xl font-semibold">Bắt đầu hành trình đầu tiên</h2>

              <p className="mx-auto mt-3 max-w-xl text-sm leading-7 text-[#6e7b78] sm:text-base">
                Chọn một tour mẫu, điền ngày khởi hành và số người. Sau khi tạo thành công, lịch trình sẽ được lưu vào
                tài khoản này.
              </p>
            ...
            <section className="mt-6 grid gap-6 md:grid-cols-2 xl:grid-cols-3">
              {itineraries.map((itinerary) => {
                ...
                return (
                  ...
                  >
                    <DeleteItineraryButton itineraryId={itinerary.id} title={itinerary.title} />
                    ...
                    >
                      <div className="relative h-52 overflow-hidden bg-[#dcebe7]">
                        {itinerary.coverImageUrl ? (
                          ...
                          <div className="grid h-full place-items-center bg-[radial-gradient(circle_at_top_right,#f7d6ca,transparent_42%),linear-gradient(135deg,#dcebe7,#f7f0e4)]">
                            <Route size={42} className="text-[#557b76]" />
                          </div>
                        )}
                      ...
                      <div className="p-5 sm:p-6">
                        ...
                        {itinerary.description ? (
                          <p className="mt-3 line-clamp-2 text-sm leading-6 text-[#6b7976]">{itinerary.description}</p>
                        ) : null}

                        <div className="mt-5 grid grid-cols-2 gap-3 text-sm text-[#5f706c]">
                          <span className="flex items-center gap-2">
                            <CalendarDays size={16} className="shrink-0 text-[#2f7773]" />

                            {formatDate(itinerary.startDate)}
                          </span>

                          <span className="flex items-center gap-2">
                            <UsersRound size={16} className="shrink-0 text-[#2f7773]" />
                            {travelerCount} người
                          </span>

                          <span className="flex items-center gap-2">
                            <BedDouble size={16} className="shrink-0 text-[#2f7773]" />
                            {itinerary.roomCount} phòng
                          </span>

                          <span className="flex min-w-0 items-center gap-2">
                            <MapPin size={16} className="shrink-0 text-[#2f7773]" />

                            <span className="truncate">{itinerary.startLocationName ?? "Chưa có điểm đi"}</span>
                          </span>
                        </div>
                        ...
                        <div className="mt-5 flex items-center justify-between gap-4 border-t border-[#e5dbcd] pt-4">
                          <span className="flex min-w-0 items-center gap-1.5 text-xs text-[#7a8784]">
                            ...
                            <span className="truncate">Cập nhật {formatUpdatedAt(itinerary.updatedAt)}</span>
                          </span>
