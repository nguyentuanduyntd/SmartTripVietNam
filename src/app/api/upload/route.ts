...
export async function POST(request: Request) {
  ...
  try {
    ...
    const uploadTypeResult = uploadTypeSchema.safeParse(formData.get("type"));
    if (!(file instanceof File)) {
      return NextResponse.json({ message: "Vui lòng chọn một file ảnh" }, { status: 400 });
    }
    if (!uploadTypeResult.success) {
      return NextResponse.json({ message: "Loại upload không hợp lệ" }, { status: 400 });
    }
