...
export async function POST(request: Request) {
  ...
  const parsedBody = cloneTourToItineraryRequestSchema.safeParse(body);

  if (!parsedBody.success) {
    return errorResponse("Dữ liệu thiết lập hành trình không hợp lệ", 400, zodErrorToFieldErrors(parsedBody.error));
  }

  try {
    const itinerary = await cloneTourToItineraryService(parsedBody.data, authResult.user.id);
