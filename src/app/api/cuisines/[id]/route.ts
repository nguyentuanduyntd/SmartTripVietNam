...
} from "@/src/services/cuisine.service";
...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  try {
    const cuisine = await updateCuisineService(parsedId.data.id, parsedBody.data);
...
export async function DELETE(_request: Request, context: RouteContext) {
  ...
  try {
    ...
    return successResponse({ id: deleted.id }, { message: "Xóa món ăn thành công" });
  } catch (error) {
    if (error instanceof CuisineNotFoundError) {
      return errorResponse(error.message, 404);
    }
