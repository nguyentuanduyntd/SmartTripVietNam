...
function createRequest(search = "") {
  return new Request(`http://localhost/api/admin/stats${search}`);
}
...
describe("GET /api/admin/stats", () => {
  ...
  it("trả 400 khi month không đúng YYYY-MM", async () => {
    ...
    const response = await GET(createRequest("?month=2026-13"));
  ...
  it("trả 200 và dữ liệu thống kê cho admin", async () => {
    ...
    const response = await GET(createRequest("?month=2026-08"));
    ...
    expect(mockedGetAdminStats).toHaveBeenCalledWith("2026-08");
  });
  ...
  it("trả 500 khi service thống kê bị lỗi", async () => {
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith("[ADMIN STATS ERROR]", error);
  });
});
