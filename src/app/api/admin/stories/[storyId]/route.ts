...
import { adminStoryIdParamSchema, deleteAdminStorySchema } from "@/src/schemas/admin-story.schema";
import { AdminStoryError, removeStoryAsAdmin } from "@/src/services/admin-story.service";
...
export async function DELETE(request: Request, context: RouteContext) {
  ...
  if (!parsedParams.success) {
    return errorResponse("Story ID không hợp lệ", 400, zodErrorToFieldErrors(parsedParams.error));
  }
  ...
  if (!parsedBody.success) {
    return errorResponse("Lý do xóa không hợp lệ", 400, zodErrorToFieldErrors(parsedBody.error));
  }

  try {
    const data = await removeStoryAsAdmin(currentUser.id, parsedParams.data.storyId, parsedBody.data);
