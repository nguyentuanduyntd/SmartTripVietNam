...
import { tourItemIdParamsSchema, updateTourItemRequestSchema } from "@/src/schemas/tour.schema";
import { deleteTourItemService, updateTourItemService } from "@/src/services/tour.service";
...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  if (!parsedId.success) {
    return errorResponse("Tour item ID không hợp lệ", 400, zodErrorToFieldErrors(parsedId.error));
  }

  const body = await request.json().catch(() => null);
  const parsedBody = updateTourItemRequestSchema.safeParse(body);
  ...
  try {
    const item = await updateTourItemService(parsedId.data.id, parsedBody.data);
...
export async function DELETE(_request: Request, context: RouteContext) {
  ...
  try {
    const deleted = await deleteTourItemService(parsedId.data.id);
