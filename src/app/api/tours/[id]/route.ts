...
import { tourIdParamsSchema, updateTourRequestSchema } from "@/src/schemas/tour.schema";
import { deleteTourService, getTourByIdService, updateTourService } from "@/src/services/tour.service";
...
export async function GET(_request: Request, context: RouteContext) {
  ...
  try {
    const tour = await getTourByIdService(parsedId.data.id, {
...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  try {
    const tour = await updateTourService(parsedId.data.id, parsedBody.data);
...
export async function DELETE(_request: Request, context: RouteContext) {
  ...
  try {
    const deleted = await deleteTourService(parsedId.data.id);
