...
export async function GET(_request: Request, context: RouteContext) {
  ...
  try {
    const community = await getTourCommunityService(parsedId.data.id, currentUser?.id ?? null);
    ...
    console.error("Không tải được dữ liệu cộng đồng của tour:", error);

    return errorResponse("Không thể tải lượt thích và nhận xét", 500);
  }
}
