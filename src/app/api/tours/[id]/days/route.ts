...
import { createStandaloneTourDayRequestSchema, tourIdParamsSchema } from "@/src/schemas/tour.schema";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  const parsedBody = createStandaloneTourDayRequestSchema.safeParse(body);
  ...
  try {
    const day = await createTourDayService(parsedId.data.id, parsedBody.data);
