...
import { setTourLikeService, TourCommunityServiceError } from "@/src/services/tour-community.service";
...
export async function PUT(request: Request, context: RouteContext) {
  ...
  const parsedBody = tourLikeRequestSchema.safeParse(body);
  ...
  try {
    const result = await setTourLikeService(parsedId.data.id, authResult.user.id, parsedBody.data.liked);

    return successResponse(result, {
      message: parsedBody.data.liked ? "Đã thích tour" : "Đã bỏ thích tour",
    });
  } catch (error) {
