...
import { createTourCommentService, TourCommunityServiceError } from "@/src/services/tour-community.service";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    const comment = await createTourCommentService(parsedId.data.id, authResult.user.id, parsedBody.data.content);
    ...
    console.error("Không thể gửi nhận xét:", error);

    return errorResponse("Không thể gửi nhận xét", 500);
  }
}
