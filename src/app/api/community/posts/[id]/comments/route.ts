...
import { communityCommentCreateSchema, communityPostIdParamsSchema } from "@/src/schemas/community.schema";
import {
...
export async function GET(_request: Request, context: RouteContext) {
  ...
  try {
    const comments = await getCommunityCommentsService(parsedId.data.id);
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    return successResponse(comment, {
      status: 201,
      message: parsedBody.data.parentId ? "Đã trả lời bình luận" : "Đã gửi bình luận",
    });
  } catch (error) {
    return handleCommunityError(error);
  }
