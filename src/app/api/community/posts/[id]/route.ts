...
import { communityPostIdParamsSchema, updateCommunityPostSchema } from "@/src/schemas/community.schema";
import {
...
export async function GET(_request: Request, context: RouteContext) {
  ...
  try {
    ...
    const post = await getCommunityPostService(parsedId.data.id, currentUser?.id ?? null);
...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  if (!parsedBody.success) {
    return errorResponse("Dữ liệu cập nhật không hợp lệ", 400, zodErrorToFieldErrors(parsedBody.error));
  }

  try {
    const updated = await updateCommunityPostService(parsedId.data.id, auth.user.id, parsedBody.data);
...
export async function DELETE(_request: Request, context: RouteContext) {
  ...
  try {
    const deleted = await deleteCommunityPostService(parsedId.data.id, auth.user.id);
