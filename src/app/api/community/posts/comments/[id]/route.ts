...
import { communityCommentIdParamsSchema, communityCommentUpdateSchema } from "@/src/schemas/community.schema";
import {
...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  if (!parsedBody.success) {
    return errorResponse("Dữ liệu bình luận không hợp lệ", 400, zodErrorToFieldErrors(parsedBody.error));
  }

  try {
    const updated = await updateCommunityCommentService(parsedId.data.id, auth.user.id, parsedBody.data.content);
...
export async function DELETE(_request: Request, context: RouteContext) {
  ...
  try {
    const deleted = await deleteCommunityCommentService(parsedId.data.id, auth.user.id);
