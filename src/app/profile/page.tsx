...
export default async function ProfilePage() {
  ...
  const profile = await findProfileById(currentUser.id);

  const email = currentUser.email ?? "Chưa cập nhật email";
  const fallbackName = currentUser.email?.split("@")[0]?.trim() || "Thành viên SmartTrip";
  const initialFullName = profile?.fullName?.trim() || fallbackName;
