...
import { Camera, MessageCircleMore, Plus, Sparkles, UsersRound } from "lucide-react";
...
export const metadata: Metadata = {
  ...
  description: "Chia sẻ trải nghiệm, hình ảnh và hành trình du lịch cùng cộng đồng SmartTripVietNam.",
};

export const dynamic = "force-dynamic";

export default async function CommunityPage() {
  const [currentUser, locations] = await Promise.all([getCurrentUser(), findCommunityLocationList()]);
    ...
    <>
      ...
      <main className="min-h-screen bg-[#f4efe6] px-4 pb-20 pt-28 text-[#173a3b] sm:px-6 sm:pt-32 lg:px-8 lg:pt-36">
        <div className="mx-auto w-full max-w-7xl">
            ...
            <div className="relative grid gap-8 lg:grid-cols-[1fr_auto] lg:items-end">
              <div className="max-w-3xl">
                ...
                <p className="mt-5 max-w-2xl text-sm leading-7 text-[#667572] sm:text-base">
                  Chia sẻ chuyến đi thật của bạn, lưu lại những hành trình hay và trao đổi kinh nghiệm với những người
                  yêu miền Trung.
                </p>
                ...
                <div className="mt-6 flex flex-wrap gap-3 text-xs font-bold text-[#5f716d]">
                  <span className="inline-flex items-center gap-2 rounded-full bg-white px-3 py-2 shadow-sm">
                    <Camera size={15} className="text-[#d85b48]" />
                    Tối đa 10 ảnh
                  </span>
                  <span className="inline-flex items-center gap-2 rounded-full bg-white px-3 py-2 shadow-sm">
                    <MessageCircleMore size={15} className="text-[#34706b]" />
                    Bình luận & trả lời
                  </span>
                  <span className="inline-flex items-center gap-2 rounded-full bg-white px-3 py-2 shadow-sm">
                    <Sparkles size={15} className="text-[#c48b28]" />
                    Chia sẻ từ Planner
                  </span>
                </div>
              </div>
          ...
          <CommunityFeed locations={locations} isAuthenticated={Boolean(currentUser)} />
        </div>
      </main>
    </>
  );
