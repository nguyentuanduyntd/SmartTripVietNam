...
export const metadata: Metadata = {
  ...
  description: "Đăng một trải nghiệm du lịch mới lên cộng đồng SmartTripVietNam.",
};
...
export default async function CommunityNewPage({ searchParams }: CommunityNewPageProps) {
  ...
  if (!user) {
    redirect("/auth/login?next=%2Fcommunity%2Fnew");
  }

  const [itineraries, locations, resolvedSearchParams] = await Promise.all([
    findUserItineraryList(user.id),
    findCommunityLocationList(),
    searchParams,
  ]);

  const rawItineraryId = resolvedSearchParams.itineraryId;

  const initialItineraryId = Array.isArray(rawItineraryId) ? (rawItineraryId[0] ?? "") : (rawItineraryId ?? "");

  const validInitialItineraryId = itineraries.some((itinerary) => itinerary.id === initialItineraryId)
    ? initialItineraryId
    : "";
      ...
      <main className="min-h-screen bg-[#f4efe6] px-4 pb-20 pt-28 text-[#173a3b] sm:px-6 sm:pt-32 lg:px-8 lg:pt-36">
        <div className="mx-auto w-full max-w-6xl">
          ...
          <section className="relative mt-6 overflow-hidden rounded-[34px] border border-white/80 bg-[#fffaf1] px-6 py-8 shadow-[0_24px_80px_rgba(23,58,59,0.10)] sm:px-9 lg:px-11 lg:py-10">
            ...
            <div className="relative flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
              <div className="max-w-3xl">
                <p className="text-xs font-extrabold uppercase tracking-[0.18em] text-[#d85b48]">Community Story</p>
                ...
                <p className="mt-4 max-w-2xl text-sm leading-7 text-[#667572]">
                  Chọn một Planner để hệ thống tự lấy hành trình, hoặc tự viết một trải nghiệm độc lập. Bạn có thể đăng
                  tối đa 10 ảnh.
                </p>
              </div>
          ...
          <CommunityCreateForm
            ...
            locations={locations}
            initialItineraryId={validInitialItineraryId}
          />
        </div>
      </main>
    </>
