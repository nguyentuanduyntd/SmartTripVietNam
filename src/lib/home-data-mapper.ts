...
function getCityIdFromLocationSlug(slug: string): CityId | null {
...
function getLocationByCityId(locations: LocationApiItem[], cityId: CityId): LocationApiItem | undefined {
  return locations.find((location) => getCityIdFromLocationSlug(location.slug) === cityId);
}

function getCityFallbackImage(cityId: CityId): string {
  return HOME_CITIES.find((city) => city.id === cityId)?.image ?? HOME_MEDIA.heroHue;
}

function formatCuisinePrice(price: number | null, locale: AppLocale): string {
  if (price === null) {
    return locale === "en" ? "Reference price" : "Giá tham khảo";
  }

  if (locale === "en") {
    return `Around ₫${new Intl.NumberFormat("en-US").format(price)}`;
  }

  return `Khoảng ${new Intl.NumberFormat("vi-VN").format(price)}đ`;
}

function formatTourPrice(price: string | null, locale: AppLocale): string {
  if (!price) {
    return locale === "en" ? "Contact us" : "Liên hệ";
  }
...
function formatTourDuration(durationDays: number, durationNights: number, locale: AppLocale): string {
  if (locale === "en") {
    const dayLabel = durationDays === 1 ? "day" : "days";

    const nightLabel = durationNights === 1 ? "night" : "nights";
...
function createDynamicCities(locations: LocationApiItem[], locale: AppLocale): HomeCity[] {
  const staticData = getLocalizedHomeStaticData(locale);

  return staticData.cities.map((staticCity) => {
    ...
    return {
      ...
      description: localizedText(
        ...
        {
          fallback: staticCity.description,
        },
      ),
...
function createCityEditorial(
  ...
  return staticData.cityEditorial.map((staticItem, index) => {
    const staticCity = staticData.cities[index];
    ...
    const destinationCount = location
      ? destinations.filter((destination) => destination.locationId === location.id).length
      : 0;
    ...
    return {
      ...
      stat:
        locale === "en"
          ? `${String(destinationCount).padStart(2, "0")} destinations`
          : `${String(destinationCount).padStart(2, "0")} điểm đến`,
    };
  });
}
...
function createDestinationCards(
  ...
  const mappedItems = destinations
    .map((destination) => {
      ...
      const destinationName = localizedText(locale, {
        vi: destination.name,
        en: destination.nameEn,
      });

      const locationName = localizedText(locale, {
        vi: location.name,
        en: location.nameEn,
      });
      ...
      const category = firstCategory
        ? localizedText(
            ...
            {
              fallback: locale === "en" ? "Destination" : "Điểm đến",
            },
          )
        : locale === "en"
          ? "Destination"
      ...
      return {
        ...
        description: localizedText(
          ...
          {
            fallback: destination.address ?? generatedDescription,
          },
        ),
  ...
  const balancedItems = staticData.cities.flatMap((city) =>
    mappedItems.filter((item) => item.city === city.id).slice(0, 2),
  );
...
function createCuisineCards(
  ...
  const mappedItems = cuisines.map((cuisine) => {
    ...
    const cuisineName = localizedText(locale, {
      vi: cuisine.name,
      en: cuisine.nameEn,
    });
    ...
    return {
      ...
      price: formatCuisinePrice(cuisine.avgPrice, locale),
      ...
      description: localizedText(
        ...
        {
          fallback: locale === "en" ? `Discover the flavors of ${cuisineName}.` : `Khám phá hương vị ${cuisineName}.`,
        },
      ),
  ...
  return mappedItems.length > 0 ? mappedItems.slice(0, 4) : staticData.cuisines;
}

function createJourneyCards(tours: TourApiItem[], locale: AppLocale): JourneyCardData[] {
  const staticData = getLocalizedHomeStaticData(locale);
  ...
  return tours.slice(0, 3).map((tour, index) => {
    const cityId = getCityIdFromLocationSlug(tour.startLocation.slug);

    const tone = JOURNEY_TONES[index % JOURNEY_TONES.length] ?? "coral";

    const tourName = localizedText(locale, {
      vi: tour.name,
      en: tour.nameEn,
    });

    const startLocationName = localizedText(locale, {
      vi: tour.startLocation.name,
      en: tour.startLocation.nameEn,
    });

    return {
      id: tour.id,

      duration: formatTourDuration(tour.durationDays, tour.durationNights, locale),
      ...
      price: formatTourPrice(tour.estimatedPrice, locale),

      image: tour.coverImageUrl ?? getCityFallbackImage(cityId ?? "hue"),
...
export function createHomeViewData(apiData: HomeApiData, locale: AppLocale): HomeViewData {
  return {
    cities: createDynamicCities(apiData.locations, locale),

    cityEditorial: createCityEditorial(apiData.locations, apiData.destinations, locale),

    featuredDestinations: createDestinationCards(apiData.locations, apiData.destinations, locale),

    cuisines: createCuisineCards(apiData.locations, apiData.destinations, apiData.cuisines, locale),

    journeys: createJourneyCards(apiData.tours, locale),
  };
}

export function getFallbackHomeViewData(locale: AppLocale): HomeViewData {
  const staticData = getLocalizedHomeStaticData(locale);

  return {
    cities: staticData.cities,

    cityEditorial: staticData.cityEditorial.map((item) => ({
      ...item,
    })),

    featuredDestinations: staticData.featuredDestinations,
