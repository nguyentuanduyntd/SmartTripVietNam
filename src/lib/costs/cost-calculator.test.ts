...
describe("cost-calculator", () => {
  describe("toSafeCostNumber", () => {
    ...
    it.each([null, undefined, -1, "-100", "không phải số", Number.NaN, Number.POSITIVE_INFINITY])(
      "trả 0 cho giá trị không hợp lệ: %s",
      (value) => {
  ...
  describe("roundCostMoney", () => {
    ...
    it.each([Number.NaN, Number.POSITIVE_INFINITY, Number.NEGATIVE_INFINITY])(
      "trả 0 khi số tiền không hữu hạn",
      (value) => {
  ...
  describe("getTravelerCostMultiplier", () => {
    it.each([
      ...
      expect(getTravelerCostMultiplier(scope, context)).toBe(expected);
    });
