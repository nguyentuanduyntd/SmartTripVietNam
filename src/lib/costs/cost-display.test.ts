...
import { formatCostFormula, formatStayFormula } from "@/src/lib/costs/cost-display";
import { formatVnd } from "@/src/lib/formatters";
import type { CostCalculationContext } from "@/src/lib/costs/cost-calculator";
...
describe("cost-display", () => {
  describe("formatCostFormula", () => {
    ...
    it.each([
      ...
      ).toBe(`${formattedPrice} × 2 × ${count} ${label}`);
    });
    ...
    it("hiển thị per_room với số phòng và số đêm", () => {
      ...
      ).toBe(`${formattedPrice} × 2 phòng × 3 đêm`);
    });
    ...
    it("dùng số đêm mặc định và tối thiểu một đêm", () => {
      ...
      ).toBe(`${formattedPrice} × 2 phòng × 2 đêm`);
      ...
      ).toBe(`${formattedPrice} × 2 phòng × 1 đêm`);
    });
  ...
  it("hiển thị công thức chi phí lưu trú", () => {
    ...
    ).toBe(`${formatVnd(500000)} × 2 phòng × 3 đêm`);
  });
});
