...
describe("OTP utilities", () => {
  ...
  describe("hashOtp", () => {
    ...
    it("tạo hash khác nhau khi secret thay đổi", () => {
      ...
      process.env.OTP_HASH_SECRET = "another-unit-test-secret";
    ...
    it("báo lỗi khi chưa cấu hình OTP_HASH_SECRET", () => {
      ...
      expect(() => hashOtp("123456")).toThrow("OTP_HASH_SECRET is not defined");
    });
  });
  ...
  describe("verifyOtpHash", () => {
    it("chấp nhận đúng OTP", () => {
      ...
      expect(verifyOtpHash("123456", expectedHash)).toBe(true);
    });

    it("từ chối OTP không đúng", () => {
      const expectedHash = hashOtp("123456");

      expect(verifyOtpHash("654321", expectedHash)).toBe(false);
    });

    it("từ chối expected hash sai độ dài", () => {
      expect(verifyOtpHash("123456", "invalid-hash")).toBe(false);
    });
  });
});
