...
describe("Redis integration", () => {
  ...
  it("ghi, đọc và thiết lập TTL cho JSON cache", async () => {
    ...
    await expect(getCachedValue(key)).resolves.toEqual({
      totalUsers: 12,
      month: "2026-08",
    });
  ...
  it("cache miss gọi loader một lần, cache hit không gọi lại", async () => {
    ...
    const databaseLoader = vi.fn().mockResolvedValue({ source: "database" });

    await expect(rememberCachedValue(key, 30, databaseLoader)).resolves.toEqual({ source: "database" });
    expect(databaseLoader).toHaveBeenCalledOnce();

    const secondLoader = vi.fn();

    await expect(rememberCachedValue(key, 30, secondLoader)).resolves.toEqual({
      source: "database",
    });
  ...
  it("xóa tất cả cache key theo prefix", async () => {
    const keys = [`${testPrefix}delete:a`, `${testPrefix}delete:b`, `${testPrefix}delete:c`];

    await Promise.all(keys.map((key, index) => redis.set(key, String(index), "EX", 30)));

    await deleteCacheByPrefix(`${testPrefix}delete:`);

    await expect(redis.mget(...keys)).resolves.toEqual([null, null, null]);
  });
});
