...
describe("redis-cache", () => {
  ...
  describe("buildCacheKey", () => {
    ...
    it("mã hóa ký tự đặc biệt và chuẩn hóa null/undefined", () => {
      ...
      ).toBe("active=true&filter=food%26stay&nullable=&optional=");
    });
  });

  describe("getCachedValue", () => {
    it("parse và trả dữ liệu khi cache hit", async () => {
      redisMock.get.mockResolvedValueOnce(JSON.stringify({ total: 12 }));

      await expect(getCachedValue<{ total: number }>("stats:2026-08")).resolves.toEqual({ total: 12 });
    });

    it("trả null khi cache miss", async () => {
      redisMock.get.mockResolvedValueOnce(null);

      await expect(getCachedValue("stats:2026-08")).resolves.toBeNull();
    });
  ...
  describe("setCachedValue", () => {
    it("serialize dữ liệu và lưu với TTL", async () => {
      ...
      await setCachedValue("stats:2026-08", { total: 12 }, 300);

      expect(redisMock.set).toHaveBeenCalledWith("stats:2026-08", JSON.stringify({ total: 12 }), "EX", 300);
    });

    it("không làm hỏng request khi Redis ghi lỗi", async () => {
      redisMock.set.mockRejectedValueOnce(new Error("Redis unavailable"));

      await expect(setCachedValue("stats:2026-08", { total: 12 }, 300)).resolves.toBeUndefined();
  ...
  describe("rememberCachedValue", () => {
    it("trả cache và không gọi loader khi cache hit", async () => {
      redisMock.get.mockResolvedValueOnce(JSON.stringify({ source: "redis" }));
      const loader = vi.fn();

      await expect(rememberCachedValue("home", 300, loader)).resolves.toEqual({
        source: "redis",
      });
    ...
    it("gọi loader và lưu cache khi cache miss", async () => {
      ...
      expect(redisMock.set).toHaveBeenCalledWith("home", JSON.stringify({ source: "database" }), "EX", 300);
    });
  ...
  describe("deleteCacheByPrefix", () => {
    it("scan và xóa toàn bộ key theo từng batch", async () => {
      redisMock.scan.mockResolvedValueOnce(["7", ["home:a", "home:b"]]).mockResolvedValueOnce(["0", []]);
      redisMock.del.mockResolvedValueOnce(2);

      await deleteCacheByPrefix("home:");

      expect(redisMock.scan).toHaveBeenNthCalledWith(1, "0", "MATCH", "home:*", "COUNT", 100);
      expect(redisMock.scan).toHaveBeenNthCalledWith(2, "7", "MATCH", "home:*", "COUNT", 100);
      expect(redisMock.del).toHaveBeenCalledWith("home:a", "home:b");
    });

    it("không làm hỏng request khi xóa cache lỗi", async () => {
      redisMock.scan.mockRejectedValueOnce(new Error("Redis unavailable"));

      await expect(deleteCacheByPrefix("home:")).resolves.toBeUndefined();
      expect(consoleErrorSpy).toHaveBeenCalledWith(
