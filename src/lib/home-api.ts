...
async function getApiData<T>(url: string, signal?: AbortSignal): Promise<T> {
  ...
  if (!response.ok || !payload || payload.success !== true) {
    const message =
      payload && "message" in payload && payload.message ? payload.message : `Không thể tải dữ liệu từ ${url}`;
...
export async function fetchHomeApiData(signal?: AbortSignal): Promise<HomeApiData> {
