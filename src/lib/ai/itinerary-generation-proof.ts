...
export function verifyAiItineraryGenerationProof(input: {
  ...
  if (!safeSignatureEqual(signature, expectedSignature)) {
    throw new AiItineraryGenerationProofError("Generation proof đã bị thay đổi hoặc không hợp lệ.");
  }
  ...
  try {
    ...
    throw new AiItineraryGenerationProofError("Không thể đọc generation proof.");
  }

  const parsed = proofPayloadSchema.safeParse(rawPayload);

  if (!parsed.success) {
    throw new AiItineraryGenerationProofError("Generation proof có cấu trúc không hợp lệ.");
  }
  ...
  if (payload.expiresAt <= now) {
    throw new AiItineraryGenerationProofError("Lịch trình AI đã hết thời gian lưu. Vui lòng tạo lại lịch trình.");
  }

  if (payload.userId !== input.userId) {
    throw new AiItineraryGenerationProofError("Generation proof không thuộc tài khoản hiện tại.");
  }

  if (payload.locationId !== input.request.locationId) {
    throw new AiItineraryGenerationProofError("Khu vực của lịch trình đã bị thay đổi.");
  }

  const currentRequestDigest = createDigest(input.request);

  if (payload.requestDigest !== currentRequestDigest) {
    throw new AiItineraryGenerationProofError("Thông tin yêu cầu đã bị thay đổi sau khi AI tạo lịch trình.");
  }

  const currentPlanDigest = createDigest(input.plan);

  if (payload.planDigest !== currentPlanDigest) {
    throw new AiItineraryGenerationProofError("Nội dung lịch trình đã bị thay đổi sau khi AI tạo.");
  }

  return {
    allowedDestinationIds: new Set(payload.allowedDestinationIds),

    allowedCuisineIds: new Set(payload.allowedCuisineIds),
