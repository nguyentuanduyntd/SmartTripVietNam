...
describe("formatters", () => {
  describe("toFiniteNumber", () => {
    ...
    it.each([null, undefined, "", "   ", "không phải số", Number.NaN, Number.POSITIVE_INFINITY])(
      "trả null cho giá trị không hợp lệ: %s",
      (value) => {
  ...
  describe("định dạng tiền và số lượng", () => {
    it("định dạng VND", () => {
      ...
      expect(formatVnd("500000")).toBe(vndFormatter.format(500000));
    });

    it("dùng fallback khi giá tiền không hợp lệ", () => {
      expect(formatVnd(null)).toBe("—");
      expect(formatVnd("không hợp lệ", "Chưa có giá")).toBe("Chưa có giá");
    });

    it("formatOptionalVnd trả null thay vì fallback", () => {
      expect(formatOptionalVnd(undefined)).toBeNull();
      expect(formatOptionalVnd(100000)).toBe(vndFormatter.format(100000));
    });
    ...
    it("định dạng quantity với số chữ số thập phân tùy chỉnh", () => {
      ...
      expect(formatQuantity("không hợp lệ", "N/A")).toBe("N/A");
    });
  });

  describe("định dạng ngày giờ", () => {
    it("giữ đúng ngày với chuỗi YYYY-MM-DD", () => {
      expect(formatVietnameseDate("2026-08-29")).toBe("29/08/2026");
    });

    it("chuyển timestamp sang múi giờ Việt Nam", () => {
      const value = "2026-08-28T18:30:00.000Z";

      expect(formatVietnameseDate(value)).toBe(dateFormatter.format(new Date(value)));
      expect(formatVietnameseDate(value)).toBe("29/08/2026");
    });

    it("hỗ trợ Date object và fallback cho ngày lỗi", () => {
      const value = new Date("2026-08-29T01:00:00.000Z");

      expect(formatVietnameseDate(value)).toBe(dateFormatter.format(value));
      expect(formatVietnameseDate(null)).toBe("—");
      expect(formatVietnameseDate("invalid-date", "N/A")).toBe("N/A");
    });
    ...
    it("định dạng ngày và giờ theo múi giờ Việt Nam", () => {
      ...
      expect(formatVietnameseDateTime(value)).toBe(dateTimeFormatter.format(dateValue));
      expect(formatVietnameseDateTime(dateValue)).toBe(dateTimeFormatter.format(dateValue));
      expect(formatVietnameseDateTime(null)).toBe("—");
      expect(formatVietnameseDateTime("invalid-date", "N/A")).toBe("N/A");
    });
  });
  ...
  describe("formatRelativeTime", () => {
    ...
    it("hiển thị ngày đầy đủ khi đã qua ít nhất 7 ngày", () => {
      ...
      expect(formatRelativeTime(value, now)).toBe(formatVietnameseDate(value));
    });
