...
import { createHomeViewData, getFallbackHomeViewData } from "@/src/lib/home-data-mapper";
...
export function useHomeData() {
  ...
  useEffect(() => {
    ...
    fetchHomeApiData(controller.signal)
      ...
      .catch((caughtError: unknown) => {
        if (!active || controller.signal.aborted || isAbortError(caughtError)) {
          return;
        }
  ...
  const viewData = useMemo(
    () => (apiData ? createHomeViewData(apiData, locale) : getFallbackHomeViewData(locale)),
    [apiData, locale],
  );
